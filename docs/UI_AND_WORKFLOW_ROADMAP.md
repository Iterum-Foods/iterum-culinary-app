...
| 2026-05-10 | **Bar checklists (opening / midday / closing / station stock):** new admin dashboard card "Bar checklists" publishes a four-list pack to `projects/{pid}/snapshots/bar_checklist_pack`; mobile Bar tab renders three checkable shift rituals + a reference station stock list; per-user daily completion held in `localStorage` and resets at midnight. One-click **Import sample** seeds a generic-bar starter pack. No new Firestore rules. See [briefings/PERSONA_HANDOFF_LOG.md](./briefings/PERSONA_HANDOFF_LOG.md). |
