...
| `users/{userId}/vendors/{vendorId}` | allow read/write per `firestore.rules` (owner or email-matched profile) | **E3a (client):** `firestore-sync.js` — `syncVendorsToFirestore` / `fetchVendorsFromFirestore` via **`resolveCatalogOwnerUserId()`** (workspace `firebaseUid` → signed-in user). **`vendorManager.js`** merges on load and pushes on save. |
| `users/{userId}/vendor_prices/{priceId}` | read/write: owner/email **or** workspace maintainer (`chef_leadership` / `purchasing` / `account_admin`) when `projectId` belongs to that account | **E3c:** `syncVendorPriceRowToFirestore`, `fetchVendorPricesFromFirestore`, **`getVendorPriceOverridesMap`**. Workspace rows also mirrored to `projects/{projectId}/vendor_prices`. |
| `projects/{projectId}/vendor_prices/{priceId}` | member read; maintainer write | **E3c mirror** for teammate-safe workspace prices (rules ship with Phase 1). |
