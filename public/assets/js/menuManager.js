class MenuManagerLegacy {
  ...
  loadFromStorage() {
    try {
      ...
      if (raw) {
        ...
        this.menu = parsed;
        // recover id counters
        const allItemIds = this.menu.categories.flatMap(c =>
          c.items.map(i => i.id)
        );
        const allCategoryIds = this.menu.categories.map(c => c.id);
        this.itemIdCounter =
          (allItemIds.length ? Math.max(...allItemIds) : 0) + 1;
        this.categoryIdCounter =
          (allCategoryIds.length ? Math.max(...allCategoryIds) : 0) + 1;
      }
    } catch {
      /* ignore */
    }
  ...
  addItem(categoryId, item) {
    const category =
      this.menu.categories.find(c => c.id === categoryId) ||
  ...
  renderCategories() {
    ...
    container.innerHTML = this.menu.categories
      .map(c => this.renderCategoryCard(c))
      .join('');
  }
  ...
  updatePricingSummary() {
    const totalItems = this.menu.categories.reduce(
      (sum, c) => sum + c.items.length,
      0
    );
    const prices = this.menu.categories.flatMap(c =>
      c.items.map(i => i.price).filter(p => p != null)
    );
  ...
  updatePreview() {
    ...
    const html = this.menu.categories
      .map(
        c => `
			<div class="mb-4">
				...
				<ul class="space-y-1">
					${c.items
            .map(
              i => `<li class="flex justify-between text-sm">
  ...
  exportMenu() {
    this.syncFromForm();
    const blob = new Blob([JSON.stringify(this.menu, null, 2)], {
...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    window.addMenuItem = () =>
      window.menuManager.addItem(
        window.menuManager.ensureDefaultCategory().id,
        { name: 'New Item' }
      );
    ...
    console.error('MenuManager init error', e);
  }
});
...
class MenuManager {
  ...
  async loadMenus() {
    ...
    try {
      const response = await fetch(
        `${this.apiBase}/menus/?project_id=${this.currentProject.id}`,
        {
      ...
      if (response.ok) {
        ...
        console.log(
          `📋 Loaded ${this.menus.length} menus for project: ${this.currentProject.name}`
        );
      } else {
  ...
  loadMenusFromStorage() {
    ...
    if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
      ...
      if (loadedMenus && Array.isArray(loadedMenus)) {
        ...
        console.log(
          `📋 Loaded ${this.menus.length} menus from user file: ${filename}`
        );
        return;
      }
    }
    ...
    console.log(
      `📋 Loaded ${this.menus.length} menus from localStorage: ${key}`
    );
  }
  ...
  saveMenusToStorage() {
    ...
    if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
      ...
      console.log(
        `💾 Saved ${this.menus.length} menus to user file: ${filename}`
      );
    } else {
      // Fallback to project-based or user-based localStorage
      const key = window.projectManager
      ...
      console.log(
        `💾 Saved ${this.menus.length} menus to localStorage: ${key}`
      );
    }
  }
  ...
  async createMenuFromImport(importResult) {
    try {
      ...
      const menuDescription = document.getElementById(
      ...
      this.showNotification(
        `Menu "${menuName}" created successfully from import!`,
        'success'
      );
  ...
  displayMenus() {
    ...
    menuContainer.innerHTML = this.menus
      .map(
        menu => `
  ...
  async saveNewMenu() {
    ...
    try {
      ...
      this.showNotification(
        `Menu "${menuData.name}" created successfully!`,
        'success'
      );
    } catch (error) {
  ...
  async saveMenuEdit(menuId) {
    ...
    try {
      ...
      this.showNotification(
        `Menu "${updates.name}" updated successfully!`,
        'success'
      );
    } catch (error) {
