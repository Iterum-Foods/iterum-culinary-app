...
class MenuItemsDatabase {
  constructor() {
    ...
    console.log(
      `📋 Menu Items Database initialized with ${this.items.length} items`
    );
  }
  ...
  saveMenuItem(menuItem) {
    ...
    const item = {
      id:
        menuItem.id ||
        `menu_item_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      name: menuItem.name,
      ...
      allergens: Array.isArray(menuItem.allergens)
        ...
        : menuItem.allergens
          ? menuItem.allergens
              ...
              .filter(a => a)
          : [],
      dietaryInfo: Array.isArray(menuItem.dietaryInfo)
  ...
  getItemsByMenuId(menuId) {
    return this.items.filter(
      item => item.menuIds && item.menuIds.includes(menuId)
    );
  }
