...
class BakersPercentageCalculator {
  ...
  setBaseIngredient(ingredientName, amount, unit = 'g') {
    ...
    console.log(
      `🍞 Base ingredient set: ${ingredientName} (${amount}${unit}) = 100%`
    );
  }
  ...
  addIngredient(name, amount, unit = 'g', category = 'other') {
    ...
    console.log(
      `📝 Added ingredient: ${name} (${amount}${unit}) = ${percentage.toFixed(1)}%`
    );
  }
  ...
  updatePercentage(ingredientName, percentage) {
    ...
    ingredient.originalAmount = this.convertFromGrams(
      newWeight,
    ...
    console.log(
      `🔄 Updated ${ingredientName}: ${percentage}% = ${ingredient.originalAmount.toFixed(2)}${ingredient.originalUnit}`
    );
  }
  ...
  scaleRecipe(scalingType, value = 1.0, targetUnit = 'g') {
    ...
    for (const [name, ingredient] of this.ingredients) {
      ...
      const scaledAmount = this.convertFromGrams(
        scaledWeight,
        ingredient.originalUnit
      );
   ...
   */
  getRecipeYields() {
    const totalWeight = Array.from(this.ingredients.values()).reduce(
  ...
  validateRecipe() {
    ...
    if (flourPercentage > 0 && waterPercentage > 0) {
      ...
      if (hydration < 50) {
        validation.warnings.push(
          'Low hydration: Consider adding more liquid for better texture'
        );
      } else if (hydration > 85) {
        validation.warnings.push(
          'High hydration: Dough may be difficult to handle'
        );
      }
    }
    ...
    if (saltPercentage < 1.5) {
      validation.warnings.push(
        'Low salt content: Consider increasing for better flavor'
      );
    ...
    if (yeastPercentage > 0) {
      if (yeastPercentage < 0.5) {
        validation.suggestions.push(
          'Low yeast: Longer fermentation time needed'
        );
  ...
  getScalingRecommendations() {
    ...
    return {
      homeKitchen: {
        ...
        reason:
          totalWeight > 2000
            ? 'Large batch - scale down for home equipment'
            : 'Good size for home kitchen'
      },
      commercial: {
        ...
        reason:
          totalWeight < 5000
            ? 'Small batch - scale up for efficiency'
            : 'Good size for commercial production'
      },
      testBatch: {
