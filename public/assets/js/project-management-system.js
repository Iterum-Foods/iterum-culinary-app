...
class ProjectManagementSystem {
    ...
    try {
      ...
      window.addEventListener('storage', event => {
        if (event.key === this.currentProjectKey) {
          console.log(
            '🔄 Project selection changed in another tab, syncing...'
          );
          this.loadCurrentProject();
          this.updateProjectUI();
        }
  ...
  loadProjects() {
    try {
      ...
      if (
        ...
        !this.projects.some(p => p.id !== 'master')
      ) {
        ...
        if (backup) {
          try {
            ...
            if (backupData.projects && backupData.projects.length > 0) {
              console.log(
                `🔄 Restoring ${backupData.projects.length} projects from backup`
              );
  ...
  createMasterProject() {
    const masterProject = {
      ...
      name: 'Master Project',
      description:
        'Complete data repository - shows all ingredients, recipes, and menus',
      type: 'master',
      status: 'active',
      createdAt: new Date().toISOString(),
      isMaster: true,
  ...
  saveProjects() {
    try {
      ...
      console.log(
        '✅ Projects saved to localStorage for user:',
        this.currentUserId
      );
  ...
  loadCurrentProject() {
    try {
      ...
      console.log(
        '📋 Final current project for user',
        this.currentUserId + ':',
  ...
  updateProjectSelectors() {
    ...
    const headerSelectorDisplay = document.querySelector(
      '.project-selector-display'
    );
    if (headerSelectorDisplay) {
      const projectNameElement = headerSelectorDisplay.querySelector(
        '.project-selector-name'
      );
      const projectIconElement = headerSelectorDisplay.querySelector(
    ...
    if (headerSelector) {
      headerSelector.innerHTML = this.projects
        .map(
          project => `
    ...
    if (mobileSelector) {
      mobileSelector.innerHTML = this.projects
        .map(
          project => `
                <option value="${project.id}" ${project.id === this.currentProject?.id ? 'selected' : ''}>
                    ${project.icon} ${project.name}
  ...
  updateProjectDisplay() {
    ...
    const projectStatusElements = document.querySelectorAll(
      '[data-project-status]'
    );
    ...
    const projectTypeElements = document.querySelectorAll(
      '[data-project-type]'
    );
  ...
  showAllData() {
    ...
    const projectFilteredElements = document.querySelectorAll(
      '[data-project-filtered]'
    );
  ...
  filterDataByProject() {
    ...
    if (window.dataTagger) {
      const projectItems = window.dataTagger.getProjectItems(
  ...
  updateProjectStats(projectItems) {
    ...
    statsElements.forEach(element => {
      const total =
        projectItems.ingredients.length +
        projectItems.recipes.length +
        projectItems.menus.length;
      element.innerHTML = `
  ...
  dispatchProjectChangeEvent() {
    ...
    console.log(
      '📡 Project change event dispatched for project:',
      this.currentProject?.name
    );
  }
  ...
  createProjectModal() {
    try {
      ...
      modal.className =
        'project-modal-overlay fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] opacity-0 transition-opacity duration-300';
  ...
  executeImport() {
    ...
    alert(
      'Import functionality will be implemented in the next phase. This will allow you to import ingredients, recipes, and menus from your master project.'
    );
    this.closeProjectModal();
  }
  ...
  ensureMasterProject() {
    ...
    try {
      // Check if master project exists
      const masterProject = this.projects.find(
        p => p.id === this.masterProjectId
      );
      ...
      if (!masterProject) {
        ...
        console.log(
          '✅ Master project already exists for user:',
          this.currentUserId
        );
        return true;
      }
  ...
  synchronizeAllProjectSelectors() {
    try {
      ...
      const allProjectSelectors = document.querySelectorAll(
        'select[id*="project"], select[class*="project"], [data-project-selector]'
      );
      ...
      allProjectSelectors.forEach(selector => {
        if (selector && this.currentProject) {
          ...
          if (selector.options.length !== this.projects.length) {
            selector.innerHTML = this.projects
              .map(
                project => `
                            <option value="${project.id}" ${project.id === this.currentProject.id ? 'selected' : ''}>
                                ${project.icon} ${project.name}
      ...
      console.log(
        `✅ Synchronized ${allProjectSelectors.length} project selectors`
      );
    } catch (error) {
  ...
  showProjectSelectionDropdown() {
    ...
    const projectContainer = document.querySelector(
      '.header-project-selector-container'
    );
    if (!projectContainer) {
      console.warn(
        '⚠️ Project selector container not found, falling back to modal'
      );
                            ...
                            <div class="flex items-center space-x-2">
                                ${
                                  project.id === this.currentProject?.id
                                    ...
                                    : '<button onclick="window.projectManager.selectProjectFromDropdown(\'' +
                                      project.id +
                                      '\')" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-xs">Select</button>'
                                }
                            </div>
                        </div>
                    `
    ...
    setTimeout(() => {
      document.addEventListener(
        'click',
        this.handleClickOutsideProjectDropdown
      );
    }, 100);
    ...
    this.handleProjectDropdownResize =
      this.handleProjectDropdownResize.bind(this);
  ...
  removeProjectSelectionDropdown() {
    ...
    if (dropdown) {
      ...
      document.removeEventListener(
        'keydown',
        this.handleEscapeKeyProjectDropdown
      );
  ...
  handleClickOutsideProjectDropdown = event => {
    ...
    if (dropdown && !dropdown.contains(event.target)) {
      ...
      const projectContainer = event.target.closest(
        '.header-project-selector-container'
      );
      if (!projectContainer) {
        this.removeProjectSelectionDropdown();
...
window.initializeProjectManagement = function () {
  if (window.projectManager && !window.projectManager.initialized) {
    try {
      ...
      console.warn(
        '⚠️ Global project management initialization failed:',
        error
      );
    }
  }
  ...
  if (window.projectManager && window.projectManager.currentProject) {
    setTimeout(() => {
      try {
        ...
        const projectSelectors = document.querySelectorAll(
          '#header-project-selector, #mobile-project-selector'
        );
...
window.forceUpdateProjectUI = function () {
  if (window.projectManager) {
    try {
      ...
      const customProjectDisplays = document.querySelectorAll(
      ...
      customProjectDisplays.forEach(element => {
        ...
        if (
          ...
          typeof element.updateProjectDisplay === 'function'
        ) {
          element.updateProjectDisplay();
        }
      });
...
function updatePageProjectDisplays(projectDetail) {
  try {
    ...
    const currentProjectElements = document.querySelectorAll(
    ...
    currentProjectElements.forEach(element => {
      if (projectDetail.project) {
        ...
        element.className =
          element.className.replace(/current-project-\w+/g, '') +
          ' current-project-' +
    ...
    projectStatusElements.forEach(element => {
      if (projectDetail.project) {
        element.textContent = projectDetail.isMaster
          ? 'Master Project'
    ...
    const projectContextHeaders = document.querySelectorAll(
      '[data-project-context-header]'
    );
    projectContextHeaders.forEach(element => {
      if (projectDetail.project) {
        element.textContent = `Working in: ${projectDetail.project.name}`;
        element.className =
          element.className.replace(/context-header-\w+/g, '') +
          ' context-header-' +
          projectDetail.project.id;
      }
    });
