class ChecklistManager {
  ...
  init() {
    ...
    this.currentUserId =
      window.authManager?.currentUser?.id || window.currentUser?.id || null;
    ...
    window.addEventListener('storage', event => {
      ...
      if (projectId === this.activeProjectId) {
        this.notify('entriesLoaded', {
          projectId,
          entries: this.getEntries(projectId)
        });
      }
    });
  ...
  registerDefaultTemplates() {
      ...
      fields: [
        ...
        },
        {
          ...
          alerts: {
            ...
            high: 41,
            message:
              'Temperature must be between 33°F and 41°F to remain within HACCP guidelines.'
          }
        },
        {
          id: 'correctiveAction',
    ...
    const sanitizerTemplate = {
      ...
      description:
        'Verify sanitizer buckets / stations are within safe ppm range.',
      category: 'foodSafety',
  ...
  async fetchRemoteEntries(projectId) {
    if (!window.firestoreSync?.getChecklistEntries) {
      return;
    }
    try {
      const remoteEntries = await window.firestoreSync.getChecklistEntries(
        projectId,
        { limit: 250 }
      );
      ...
      console.warn(
        '⚠️ Unable to fetch checklist entries from Firestore:',
        error
      );
    }
  }
  ...
  mergeRemoteEntries(projectId, remoteEntries) {
    ...
    remoteEntries.forEach(remote => {
      ...
      const remoteUpdatedAt = this.parseTimestamp(
        remote.updatedAt || remote.timestamp
      );
      const existingUpdatedAt = this.parseTimestamp(
      ...
      if (!existing || remoteUpdatedAt > existingUpdatedAt) {
        merged.set(remote.id, {
          ...
          updatedAt:
            remote.updatedAt || remote.timestamp || new Date().toISOString(),
  ...
  addEntry(templateId, formData, options = {}) {
    ...
    const currentUser =
      window.authManager?.currentUser || window.currentUser || {};
    const ownerId =
      options.ownerId || currentUser.id || currentUser.userId || 'anonymous';
    ...
    if (
      evaluation.requiresCorrectiveAction &&
      (!formData.correctiveAction || !formData.correctiveAction.trim())
    ) {
      throw new Error(
        'Corrective action is required when readings fall outside accepted range.'
      );
    }
    ...
    if (window.firestoreSync?.saveChecklistEntry) {
      window.firestoreSync
        ...
        .catch(error => {
          console.warn(
            '⚠️ Failed to sync checklist entry with Firestore:',
            error
          );
        });
    }
  ...
  getTemplateStatus(templateId, projectId = this.getActiveProjectId()) {
    ...
    const entries = this.getEntries(projectId).filter(
