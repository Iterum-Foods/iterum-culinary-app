...
if (typeof window.RecipeLibrary !== 'undefined') {
  ...
  class RecipeLibrary {
    ...
    addRecipeChange(action, recipeName, description = '') {
      ...
      let changes = JSON.parse(
        localStorage.getItem('iterum_recipe_changes') || '[]'
      );
      changes.push(change);
    ...
    setupLibraryControls() {
      ...
      if (searchInput) {
        searchInput.addEventListener(
          'input',
          debounce(() => this.filterRecipes(), 300)
        );
      }
      ...
      for (let item of this.uploadQueue) {
        try {
          ...
          if (recipe) {
            ...
            this.addRecipeChange(
              'Added',
              recipe.name,
              'Recipe imported from file'
            );
    ...
    updateUploadProgress() {
      ...
      const processing = this.uploadQueue.filter(
    ...
    showUploadComplete() {
      ...
      if (completed > 0) {
        this.showNotification(
          `Successfully imported ${completed} recipes!`,
          'success'
        );
      }
      ...
      if (failed > 0) {
        this.showNotification(
          `${failed} recipes failed to import. Check the console for details.`,
          'error'
        );
      }
    ...
    loadRecipeLibrary() {
      ...
      if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
        ...
        this.recipes = JSON.parse(
          localStorage.getItem(`recipe_library_${currentUser.id}`) || '[]'
        );
      }
      this.displayRecipes();
    }
    ...
    saveRecipeLibrary() {
      ...
      if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
        ...
        localStorage.setItem(
          `recipe_library_${currentUser.id}`,
    ...
    getFilteredRecipes() {
      ...
      const searchTerm = document
        .getElementById('recipe-library-search')
        ?.value.toLowerCase();
      if (searchTerm) {
      ...
      // Cuisine filter
      const cuisineFilter = document.getElementById(
      ...
      input.onchange = e => {
        ...
        reader.onload = e => {
          try {
            ...
            this.showNotification(
              `Imported ${importedRecipes.length} recipes successfully`,
              'success'
            );
          } catch (error) {
  ...
  class RecipeReviewManager {
    ...
    displayPendingReviews() {
      ...
      if (this.pendingRecipes.length === 0) {
        container.innerHTML =
          '<div class="text-center py-12"><div class="text-6xl mb-4">✅</div><h3 class="text-xl font-semibold text-gray-700 mb-2">No recipes pending review</h3><p class="text-gray-500">All uploaded recipes have been reviewed!</p></div>';
        return;
      }
      container.innerHTML = '';
        ...
        btn.onclick = async () => {
          ...
          if (source === 'Uploaded') {
            ...
            const payload = {
              ...
              instructions: (recipe.instructions || recipe.steps || []).map(
  ...
  async function parseAndPreviewFiles(
    files,
    previewContainer,
    progressBar,
    progressText
    ...
    // Show preview
    if (previewContainer) {
      previewContainer.innerHTML = parsedResults
        .map(r => {
          if (r.error) {
  ...
  RecipeLibrary.prototype.handleFileUpload = async function (event, inputId) {
    ...
    // Parse and preview
    const parsedResults = await parseAndPreviewFiles(
      Array.from(files),
      previewContainer,
      progressBar,
      progressText
  ...
  if (confirmImportBtn) {
    confirmImportBtn.addEventListener('click', () => {
      ...
      window.recipeLibrary.showNotification(
        `Imported ${pending.length} recipes!`,
        'success'
      );
    });
  }
