...
class UserDataManager {
  ...
  initializeUserIngredients(userId) {
    ...
    const defaultIngredients = [
      ...
      },
      {
        ...
        allergens: [],
        nutritional_info: { carbs: 28, calories: 130 },
        created_at: new Date().toISOString()
      },
      {
        id: 18,
        ...
        category: 'Grains',
        default_unit: 'g',
        description: 'Italian staple',
        allergens: ['gluten'],
        nutritional_info: { carbs: 25, calories: 131 },
        created_at: new Date().toISOString()
      },
      {
        id: 19,
    ...
    console.log(
      `Initialized ${defaultIngredients.length} default ingredients for user`
    );
  }
  ...
  initializeUserEquipment(userId) {
    ...
    console.log(
      `Initialized ${defaultEquipment.length} default equipment for user`
    );
  }
  ...
  importUserData(data) {
    ...
    try {
      if (data.ingredients) {
        this.saveUserIngredients(data.ingredients);
      }
      if (data.equipment) {
        this.saveUserEquipment(data.equipment);
      }
      if (data.recipes) {
        this.saveUserRecipes(data.recipes);
      }
      if (data.inventory) {
        this.saveUserInventory(data.inventory);
      }
      if (data.haccp_readings) {
        this.saveUserHACCPReadings(data.haccp_readings);
      }
      if (data.journal_entries) {
        this.saveUserJournalEntries(data.journal_entries);
      }
      if (data.recipe_drafts) {
        this.saveUserDrafts(data.recipe_drafts);
      }
