...
class SmartMenuParser {
  ...
  parseMenuText(text, options = {}) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (isLikelyItem) {
        ...
        const description = this.extractInlineDescription(
          line,
          itemName,
          priceInfo
        );
        ...
        if (consecutiveDescriptionLines < 3) {
  ...
  detectCategoryHeader(line) {
    ...
    for (const pattern of this.categoryPatterns) {
      ...
      if (match) {
        ...
        if (
          this.isKnownCategory(categoryName) ||
          this.looksLikeCategoryName(categoryName)
        ) {
  ...
  looksLikeCategoryName(text) {
    ...
    if (this.extractPrice(text).price) {
      return false;
    }

    const uppercaseRatio = (text.match(/[A-Z]/g) || []).length / text.length;
    if (uppercaseRatio > 0.5 && text.length < 25) {
      return true;
    }
  ...
  isLikelyMenuItem(line, nextLine, priceInfo) {
    ...
    if (this.itemPrefixes.some(prefix => line.startsWith(prefix))) {
      return true;
    }
    ...
    if (
      ...
      !this.extractPrice(nextLine).price
    ) {
      return true;
    }
    ...
    if (
      nextLine &&
      this.extractPrice(nextLine).price &&
      !this.containsFoodKeywords(nextLine)
    ) {
      return true;
    }
  ...
  isLikelyDescription(line) {
    ...
    if (this.extractPrice(line).price) {
      return false;
    }
    ...
    if (descriptiveWords.some(word => lowerLine.includes(word))) {
      return true;
    }
  ...
  finalizeItem(item, options) {
    ...
    if (options.detectDietary !== false) {
      item.dietaryInfo = this.extractDietaryInfo(
        item.rawLine + ' ' + item.description
      );
    }

    // Extract allergens
    item.allergens = this.extractAllergens(
      item.rawLine + ' ' + item.description
    );
  ...
  extractAllergens(text) {
    ...
    const allergenList = [
      {
        name: 'Dairy',
        keywords: ['milk', 'cheese', 'cream', 'butter', 'dairy']
      },
      {
        name: 'Nuts',
        keywords: ['nuts', 'almond', 'walnut', 'peanut', 'cashew', 'pecan']
      },
      {
        name: 'Shellfish',
        keywords: ['shellfish', 'shrimp', 'lobster', 'crab', 'oyster']
      },
  ...
  autoCategorize(name, description) {
    ...
    const categoryRules = [
      ...
      },
      {
        ...
        keywords: ['soup', 'salad', 'chowder', 'bisque', 'gazpacho', 'caesar']
      },
      {
      ...
      {
        category: 'Poultry',
        keywords: ['chicken', 'turkey', 'duck', 'poultry']
      },
      {
        category: 'Pork & Lamb',
        keywords: ['pork', 'lamb', 'ribs', 'chop', 'bacon']
      },
      { category: 'Pizza', keywords: ['pizza', 'flatbread'] },
      {
        category: 'Sandwiches & Wraps',
        keywords: ['sandwich', 'wrap', 'panini', 'sub', 'hoagie']
      },
      {
        category: 'Desserts',
        keywords: [
