...
class AIIntegration {
  ...
  setupEventListeners() {
    ...
    const fileInputs = document.querySelectorAll(
      'input[type="file"][data-ai-enhanced]'
    );
  ...
  addAIButtonsToRecipeForms() {
    ...
    recipeForms.forEach(form => {
      ...
      if (form.querySelector('[data-ai-action]')) {
        return;
      }

      const buttonContainer =
        form.querySelector('.form-actions, .button-group') || form;
  ...
  updateFileUploadInputs() {
    ...
    fileInputs.forEach(input => {
      if (!input.hasAttribute('data-ai-enhanced')) {
        ...
        const container =
          input.closest('.file-upload-container') || input.parentElement;
  ...
  createAIButton(action, text, tooltip) {
    ...
    button.type = 'button';
    button.className =
      'ai-button bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors';
  ...
  async handleRecipeParsing(event) {
    ...
    const textArea = form.querySelector(
      'textarea[name="recipe_text"], textarea[name="text"]'
    );
    ...
    try {
      ...
      if (result.success) {
        ...
        this.showSuccess(
          `Recipe parsed successfully! Confidence: ${(result.confidence * 100).toFixed(1)}%`
        );
      } else {
  ...
  async handleMenuParsing(event) {
    ...
    const form = event.target.closest('form');
    const textArea = form.querySelector(
      'textarea[name="menu_text"], textarea[name="text"]'
    );
    ...
    try {
      ...
      if (result.success) {
        ...
        this.showSuccess(
          `Menu parsed successfully! Confidence: ${(result.confidence * 100).toFixed(1)}%`
        );
      } else {
        throw new Error('AI parsing failed');
      }
  ...
  async handleAIFileUpload(event) {
    ...
    const aiProcess = event.target
      .closest('.file-upload-container')
      .querySelector('[data-ai-process]').checked;
    if (!aiProcess) {
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        this.showSuccess(
          `File processed with AI! Confidence: ${(result.confidence * 100).toFixed(1)}%`
        );
      } else {
  ...
  populateIngredients(form, ingredients) {
    const ingredientsContainer = form.querySelector(
  ...
  populateInstructions(form, instructions) {
    const instructionsContainer = form.querySelector(
  ...
  populateMenuSections(form, sections) {
    const sectionsContainer = form.querySelector(
      '.menu-sections-container, #menu-sections'
    );
  ...
  extractRecipeData(form) {
    ...
    ingredientItems.forEach(item => {
      const amount =
        item.querySelector('[name^="ingredient_amount"]')?.value || '';
  ...
  showRecipeCreationModal() {
    ...
    const methodInputs = modal.querySelectorAll(
      'input[name="creation_method"]'
    );
  ...
  async handleCreateRecipe(modal) {
    const method = modal.querySelector(
      'input[name="creation_method"]:checked'
    ).value;
    ...
    try {
      ...
      if (method === 'ingredients') {
        const ingredients = modal
          .querySelector('[name="ingredients"]')
          .value.split(',')
          .map(i => i.trim());
        response = await fetch(
          `${this.apiBase}/recipe/create-from-ingredients`,
          {
            method: 'POST',
            headers: {
  ...
  showAIDisabledMessage() {
    ...
    message.className =
      'ai-disabled-message bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-4';
    message.innerHTML = `
            <div class="flex items-center">
  ...
  showAIError(error) {
    const message = document.createElement('div');
    message.className =
      'ai-error-message bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4';
    message.innerHTML = `
            <div class="flex items-center">
