...
class RecipeScaler {
  ...
  setupEventListeners() {
    ...
    if (targetServings) {
      targetServings.addEventListener('input', () =>
  ...
  scaleRecipe() {
    ...
    if (this.targetServings === this.originalServings) {
      this.showNotification(
        'Target servings same as original - no scaling needed',
        'info'
      );
      return;
    }
    ...
    this.scaledIngredients = this.scaleIngredients(
      this.originalIngredients,
  ...
  scaleIngredients(ingredients, scalingFactor) {
    return ingredients.map(ingredient => {
      ...
      const originalQuantity = parseFloat(
        ingredient.amount || ingredient.quantity || 0
      );
  ...
  convertUnits(quantity, unit) {
    const conversions = {
      ...
      g: qty => {
        if (qty >= 1000) {
          return { quantity: (qty / 1000).toFixed(2), unit: 'kg' };
        }
        if (qty >= 453.592) {
          return { quantity: (qty / 453.592).toFixed(2), unit: 'lb' };
        }
        return { quantity: qty.toFixed(1), unit: 'g' };
      },
      kg: qty => {
        if (qty >= 2.20462) {
          return { quantity: (qty * 2.20462).toFixed(2), unit: 'lb' };
        }
        return { quantity: qty.toFixed(2), unit: 'kg' };
      },
      lb: qty => {
        if (qty >= 16) {
          return { quantity: (qty / 16).toFixed(2), unit: 'oz' };
        }
      ...
      ml: qty => {
        if (qty >= 1000) {
          return { quantity: (qty / 1000).toFixed(2), unit: 'L' };
        }
        if (qty >= 236.588) {
          return { quantity: (qty / 236.588).toFixed(2), unit: 'cup' };
        }
        if (qty >= 14.7868) {
          return { quantity: (qty / 14.7868).toFixed(2), unit: 'tbsp' };
        }
        if (qty >= 4.92892) {
          return { quantity: (qty / 4.92892).toFixed(2), unit: 'tsp' };
        }
        return { quantity: qty.toFixed(1), unit: 'ml' };
      },
      L: qty => {
        if (qty >= 4.22675) {
          return { quantity: (qty * 4.22675).toFixed(2), unit: 'cup' };
        }
        return { quantity: qty.toFixed(2), unit: 'L' };
      },
      cup: qty => {
        if (qty >= 16) {
          return { quantity: (qty * 16).toFixed(2), unit: 'tbsp' };
        }
        if (qty >= 48) {
          return { quantity: (qty * 48).toFixed(2), unit: 'tsp' };
        }
        return { quantity: qty.toFixed(2), unit: 'cup' };
      },
      tbsp: qty => {
        if (qty >= 3) {
          return { quantity: (qty / 3).toFixed(2), unit: 'tbsp' };
        }
  ...
  displayScalingResults() {
    ...
    summaryDiv.className =
      'mt-3 p-3 bg-blue-50 rounded border-l-4 border-blue-400';
  ...
  applyScaling() {
    ...
    this.showNotification(
      `Recipe scaled to ${this.targetServings} servings!`,
      'success'
    );
    this.hideScalingResults();
  }
  ...
  printScaledRecipe() {
    ...
    const recipeTitle =
      document.getElementById('recipe-title')?.value || 'Scaled Recipe';
  ...
  generatePrintContent(title, description) {
    const ingredientsList = this.scaledIngredients
      .map(
        ing => `<li>${ing.scaledQuantity} ${ing.scaledUnit} ${ing.name}</li>`
      )
      .join('');
  ...
  saveScalingHistory() {
    const scalingRecord = {
      ...
      recipeTitle:
        document.getElementById('recipe-title')?.value || 'Unknown Recipe'
    };
    ...
    localStorage.setItem(
      ...
      JSON.stringify(this.scalingHistory)
    );
  }
