...
class UserControlledStorage {
    ...
    try {
      ...
      if (backupKeys.length > this.maxBackups) {
        ...
        const keysToRemove = sortedKeys.slice(
          0,
          backupKeys.length - this.maxBackups
        );
  ...
  async exportAllUserData() {
    try {
      ...
      const exportData = {
        ...
        calendarEvents:
          this.loadUserFile(`${userId}/calendar_events.json`) || [],
      ...
      Object.keys(importData).forEach(key => {
        if (key !== 'exportInfo' && key !== 'dataTypes' && key !== 'metadata') {
          if (
            ...
            typeof importData[key] === 'object'
          ) {
            const success = this.saveUserFile(
              `${userId}/${key}.json`,
              importData[key]
            );
      ...
      Object.keys(localStorage).forEach(key => {
        if (key.startsWith('user_file_') || key.startsWith('system_file_')) {
          ...
          if (
            backup.created &&
            (!stats.lastBackup ||
              new Date(backup.created) > new Date(stats.lastBackup))
          ) {
  ...
  createStorageControlPanel() {
    ...
    panel.className =
      'storage-control-panel fixed bottom-4 right-4 bg-white rounded-lg shadow-lg border p-4 max-w-sm';
    panel.innerHTML = `
  ...
  updateStorageStats() {
    ...
    if (stats) {
      ...
      if (sizeEl) {
        sizeEl.textContent = stats.totalSizeMB + ' MB';
      }
      if (backupsEl) {
        backupsEl.textContent = stats.backupCount;
      }
    }
  }
...
window.showStorageControl = () =>
  userControlledStorage.createStorageControlPanel();
