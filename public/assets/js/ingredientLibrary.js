...
class IngredientLibrary {
  ...
  setupEventListeners() {
    ...
    const addFirstIngredientBtn = document.getElementById(
      'add-first-ingredient'
    );
    if (addFirstIngredientBtn) {
      addFirstIngredientBtn.addEventListener('click', () =>
        this.showIngredientForm()
      );
    }
    ...
    if (searchInput) {
      searchInput.addEventListener(
        'input',
        debounce(() => this.filterIngredients(), 300)
      );
    }
    ...
    // Display options
    const displayOptions = document.getElementById(
      'ingredient-display-options'
    );
    if (displayOptions) {
  ...
  async loadIngredients() {
    try {
      ...
      const userId =
        window.userDataManager && window.userDataManager.getCurrentUserId
          ? window.userDataManager.getCurrentUserId()
          : JSON.parse(localStorage.getItem('current_user') || '{}').id || null;
      if (!token) {
  ...
  loadFromLocalStorage() {
    ...
    if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
      ...
      console.log(
        `🥬 Loaded ${this.ingredients.length} ingredients from user file`
      );
    } else {
      // Fallback to global storage for backward compatibility
      this.ingredients = JSON.parse(
        localStorage.getItem('iterum_ingredients') || '[]'
      );
      console.log(
        `🥬 Loaded ${this.ingredients.length} ingredients from localStorage`
      );
    }
    this.updateIngredientCount();
    this.displayIngredients();
  ...
  getFormData() {
    ...
    const description =
      document.getElementById('ingredient-description')?.value.trim() || '';

    // Get vendor information
    const primaryVendor =
      document.getElementById('ingredient-primary-vendor')?.value.trim() || '';
    const alternateVendors =
      document.getElementById('ingredient-alternate-vendors')?.value.trim() ||
      '';
    const vendorSKU =
      document.getElementById('ingredient-vendor-sku')?.value.trim() || '';
    const unitPrice =
      parseFloat(document.getElementById('ingredient-unit-price')?.value) || 0;
    const packSize =
      document.getElementById('ingredient-pack-size')?.value.trim() || '';
    const leadTime =
      document.getElementById('ingredient-lead-time')?.value.trim() || '';
    const minimumOrder =
      document.getElementById('ingredient-minimum-order')?.value.trim() || '';
    ...
    const nutritionalInfo = {
      calories:
        parseFloat(document.getElementById('nutrition-calories')?.value) || 0,
      protein:
        parseFloat(document.getElementById('nutrition-protein')?.value) || 0,
      ...
      sodium:
        parseFloat(document.getElementById('nutrition-sodium')?.value) || 0,
    ...
    const vendorInfo = {
      ...
      alternateVendors: alternateVendors
        ? alternateVendors.split(',').map(v => v.trim())
        : [],
      vendorSKU: vendorSKU,
      unitPrice: unitPrice,
      packSize: packSize,
  ...
  showIngredientForm(ingredient = null) {
    ...
    if (ingredient) {
      ...
      document
        .querySelectorAll('#ingredient-form input[type="checkbox"]')
        .forEach(cb => {
      ...
      document.getElementById('nutrition-sodium').value =
        nutrition.sodium || '';
  ...
  changeViewMode(buttonId) {
    ...
    document
      .querySelectorAll(
        '#ingredient-grid-view, #ingredient-list-view, #ingredient-table-view'
      )
      .forEach(btn => {
  ...
  renderNutritionalInfo(ingredient) {
    if (this.displayMode !== 'all' && this.displayMode !== 'nutritional') {
      return '';
    }
  ...
  renderAllergenInfo(ingredient) {
    if (this.displayMode !== 'all' && this.displayMode !== 'allergens') {
      return '';
    }
  ...
  renderAllergenBadges(ingredient) {
    const allergens = ingredient.allergens || [];
    if (allergens.length === 0) {
      return '<span class="text-sm text-gray-500">None</span>';
    }
  ...
  saveToLocalStorage() {
    ...
    if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
      ...
      console.log(
        `💾 Saved ${this.ingredients.length} ingredients to user file`
      );
    } else {
      // Fallback to global storage for backward compatibility
      ...
      console.log(
        `💾 Saved ${this.ingredients.length} ingredients to localStorage`
      );
    }
  }
  ...
  loadDefaultIngredientDatabase() {
    ...
    const defaultIngredients = [
      // Produce
      {
        ...
        allergens: ['gluten'],
        nutritional_info: { protein: 10, carbs: 76, fat: 1, calories: 364 },
        created_at: new Date().toISOString()
      },
      {
        id: 2,
        ...
        allergens: [],
        nutritional_info: { carbs: 100, calories: 387 },
        created_at: new Date().toISOString()
      },
      {
        id: 3,
      ...
      },
      {
        ...
        category: 'Protein',
        default_unit: 'g',
        description: 'Lean protein, versatile',
        allergens: [],
        nutritional_info: { protein: 31, calories: 165 },
        created_at: new Date().toISOString()
      },
      {
        id: 12,
        ...
        category: 'Protein',
        default_unit: 'g',
        description: 'Versatile meat, high fat content',
        allergens: [],
        nutritional_info: { protein: 26, fat: 15, calories: 250 },
        created_at: new Date().toISOString()
      },
      {
        id: 13,
        ...
        allergens: [],
        nutritional_info: { carbs: 78, calories: 345 },
        created_at: new Date().toISOString()
      },
      {
        id: 14,
        ...
        category: 'Grains',
        default_unit: 'g',
        description: 'Durum wheat, basic noodle',
        allergens: ['gluten'],
        nutritional_info: { carbs: 71, protein: 13, calories: 371 },
        created_at: new Date().toISOString()
      },
      {
        id: 15,
      ...
      },
      {
        ...
        category: 'Produce',
        default_unit: 'each',
        description: 'Starchy, good for mashing/frying',
        allergens: [],
        nutritional_info: { carbs: 37, calories: 161 },
        created_at: new Date().toISOString()
      },
      {
        id: 17,
        ...
        category: 'Produce',
        default_unit: 'each',
        description: 'Root vegetable, sweet',
        allergens: [],
        nutritional_info: { carbs: 9, calories: 41 },
        created_at: new Date().toISOString()
      },
      {
        id: 18,
        ...
        category: 'Produce',
        default_unit: 'stalks',
        description: 'Aromatic, crisp',
        allergens: [],
        nutritional_info: { calories: 6 },
        created_at: new Date().toISOString()
      },
      {
        id: 19,
        ...
        category: 'Produce',
        default_unit: 'each',
        description: 'Crunchy, mild flavor',
        allergens: [],
        nutritional_info: { calories: 20 },
        created_at: new Date().toISOString()
      },
      {
        id: 20,
        ...
        category: 'Produce',
        default_unit: 'each',
        description: 'Citrus, acidic',
        allergens: [],
        nutritional_info: { calories: 17 },
        created_at: new Date().toISOString()
      },
      {
        id: 21,
      ...
      },
      {
        ...
        category: 'Herbs',
        default_unit: 'g',
        description: 'Fresh herb, bright flavor',
        allergens: [],
        nutritional_info: { calories: 23 },
        created_at: new Date().toISOString()
      },
      {
        id: 23,
        ...
        category: 'Herbs',
        default_unit: 'g',
        description: 'Fresh herb, earthy flavor',
        allergens: [],
        nutritional_info: { calories: 36 },
        created_at: new Date().toISOString()
      },
      {
        id: 24,
        ...
        category: 'Herbs',
        default_unit: 'g',
        description: 'Aromatic herb, woody stems',
        allergens: [],
        nutritional_info: { calories: 101 },
        created_at: new Date().toISOString()
      },
      {
        id: 25,
        ...
        category: 'Herbs',
        default_unit: 'g',
        description: 'Aromatic herb, strong flavor',
        allergens: [],
        nutritional_info: { calories: 131 },
        created_at: new Date().toISOString()
      },
      {
        id: 26,
        ...
        category: 'Herbs',
        default_unit: 'each',
        description: 'Aromatic, used whole',
        allergens: [],
        nutritional_info: { calories: 313 },
        created_at: new Date().toISOString()
      },
      {
        id: 27,
        ...
        category: 'Spices',
        default_unit: 'g',
        description: 'Earthy, warm spice',
        allergens: [],
        nutritional_info: { calories: 375 },
        created_at: new Date().toISOString()
      },
      {
        id: 28,
        ...
        category: 'Spices',
        default_unit: 'g',
        description: 'Citrusy, mild spice',
        allergens: [],
        nutritional_info: { calories: 298 },
        created_at: new Date().toISOString()
      },
      {
        id: 29,
        ...
        category: 'Spices',
        default_unit: 'g',
        description: 'Mild chili powder',
        allergens: [],
        nutritional_info: { calories: 282 },
        created_at: new Date().toISOString()
      },
      {
        id: 30,
        ...
        category: 'Spices',
        default_unit: 'g',
        description: 'Spicy, blend of chili peppers',
        allergens: [],
        nutritional_info: { calories: 282 },
        created_at: new Date().toISOString()
      },
      {
        ...
        category: 'Herbs',
        default_unit: 'g',
        description: 'Mediterranean herb, pungent',
        allergens: [],
        nutritional_info: { calories: 265 },
        created_at: new Date().toISOString()
      },
      {
        id: 32,
      ...
      },
      {
        ...
        category: 'Produce',
        default_unit: 'g',
        description: 'Pungent, spicy root',
        allergens: [],
      ...
      },
      {
        ...
        category: 'Condiments',
        default_unit: 'ml',
        description: 'Umami, salty',
        allergens: ['soy'],
        nutritional_info: { sodium: 879, calories: 53 },
        created_at: new Date().toISOString()
      },
      {
        id: 35,
      ...
      },
      {
        ...
        category: 'Condiments',
        default_unit: 'g',
        description: 'Sharp, tangy',
        allergens: [],
        nutritional_info: { calories: 66 },
        created_at: new Date().toISOString()
      },
      {
        id: 38,
        ...
        allergens: [],
        nutritional_info: { carbs: 82, calories: 304 },
        created_at: new Date().toISOString()
      },
      {
        id: 39,
        ...
        category: 'Sweeteners',
        default_unit: 'ml',
        description: 'Natural sweetener, distinct flavor',
        allergens: [],
        nutritional_info: { carbs: 67, calories: 260 },
        created_at: new Date().toISOString()
      },
      {
        id: 40,
        ...
        default_unit: 'slices',
        description: 'Tangy, chewy',
        allergens: ['gluten'],
        nutritional_info: { carbs: 49, protein: 9, calories: 289 },
        created_at: new Date().toISOString()
      },
      {
        id: 41,
        ...
        category: 'Dairy',
        default_unit: 'g',
        description: 'Hard cheese, sharp flavor',
        allergens: ['dairy'],
        nutritional_info: { protein: 25, fat: 33, calories: 403 },
        created_at: new Date().toISOString()
      },
      {
        id: 42,
        ...
        category: 'Dairy',
        default_unit: 'g',
        description: 'Hard, salty, umami',
        allergens: ['dairy'],
        nutritional_info: { protein: 38, fat: 29, calories: 431 },
        created_at: new Date().toISOString()
      },
      {
        id: 43,
        ...
        category: 'Dairy',
        default_unit: 'g',
        description: 'Tangy, creamy dairy product',
        allergens: ['dairy'],
        nutritional_info: { protein: 10, calories: 59 },
        created_at: new Date().toISOString()
      },
      {
        id: 44,
      ...
      },
      {
        ...
        category: 'Protein',
        default_unit: 'g',
        description: 'Plant-based protein, absorbs flavors',
        allergens: ['soy'],
        nutritional_info: { protein: 8, calories: 76 },
        created_at: new Date().toISOString()
      },
      {
        id: 46,
        ...
        category: 'Legumes',
        default_unit: 'g',
        description: 'Earthy, hearty',
        allergens: [],
        nutritional_info: { protein: 9, carbs: 20, calories: 116 },
        created_at: new Date().toISOString()
      },
      {
        id: 47,
        ...
        category: 'Legumes',
        default_unit: 'g',
        description: 'Earthy, versatile',
        allergens: [],
        nutritional_info: { protein: 8, carbs: 23, calories: 132 },
        created_at: new Date().toISOString()
      },
      {
        id: 48,
        ...
        category: 'Legumes',
        default_unit: 'g',
        description: 'Nutty, versatile legume',
        allergens: [],
        nutritional_info: { protein: 9, carbs: 27, calories: 164 },
        created_at: new Date().toISOString()
      },
      {
        id: 49,
        ...
        category: 'Grains',
        default_unit: 'g',
        description: 'Complete protein, nutty',
        allergens: [],
        nutritional_info: { protein: 4, carbs: 22, calories: 120 },
        created_at: new Date().toISOString()
      },
      {
        id: 50,
        ...
        category: 'Grains',
        default_unit: 'g',
        description: 'Whole grain, breakfast staple',
        allergens: [],
        nutritional_info: { protein: 13, carbs: 68, calories: 389 },
        created_at: new Date().toISOString()
      }
    ];
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (ingredients.length > 0) {
          console.log(
            `Loaded ${ingredients.length} ingredients from default database`
          );
  ...
  parseAllergens(allergenText) {
    ...
    if (allergenTextLower.includes('gluten')) {
      allergens.push('gluten');
    }
    if (allergenTextLower.includes('dairy')) {
      allergens.push('dairy');
    }
    if (allergenTextLower.includes('nuts')) {
      allergens.push('nuts');
    }
    if (allergenTextLower.includes('eggs')) {
      allergens.push('eggs');
    }
    if (allergenTextLower.includes('soy')) {
      allergens.push('soy');
    }
    if (allergenTextLower.includes('shellfish')) {
      allergens.push('shellfish');
    }
    if (allergenTextLower.includes('fish')) {
      allergens.push('fish');
    }
    if (allergenTextLower.includes('sulfites')) {
  ...
  processCSVFile(file) {
    ...
    reader.onload = e => {
      ...
      for (let i = 1; i < lines.length; i++) {
        if (lines[i].trim()) {
          ...
          headers.forEach((header, index) => {
            if (values[index]) {
              ingredient[header.toLowerCase().replace(/\s+/g, '_')] =
                values[index];
            }
          });
      ...
      this.showNotification(
        `Imported ${importedIngredients.length} ingredients successfully!`,
        'success'
      );
    };
  ...
  showAddViaLinkModal() {
    ...
    processBtn.addEventListener('click', () =>
      this.processIngredientLink(modal)
    );
  ...
  async processIngredientLink(modal) {
    ...
    try {
      ...
      modal.remove();
      this.showNotification(
        `Added ingredient from link: ${ingredientName}`,
        'success'
      );
    } catch (error) {
      console.error('Error processing link:', error);
      this.showNotification(
        'Failed to process link. Please try manual entry.',
        'error'
      );
  ...
  showEditAllModal() {
    ...
    deleteSelectedBtn.addEventListener('click', () =>
      this.deleteSelectedIngredients(modal)
    );
  ...
  showBulkCategoryModal(parentModal) {
    ...
    applyBtn.addEventListener('click', () => {
      ...
      parentModal.remove();
      this.showNotification(
        `Updated category for ${selectedIds.length} ingredients`,
        'success'
      );
    });
  }
  ...
  showBulkUnitModal(parentModal) {
    ...
    applyBtn.addEventListener('click', () => {
      ...
      parentModal.remove();
      this.showNotification(
        `Updated unit for ${selectedIds.length} ingredients`,
        'success'
      );
    });
  }
  ...
  deleteSelectedIngredients(parentModal) {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${selectedIds.length} ingredients?`
      )
    ) {
      return;
    }
    ...
    parentModal.remove();
    this.showNotification(
      `Deleted ${selectedIds.length} ingredients`,
      'success'
    );
  }
  ...
  async previewURLImport() {
    ...
    try {
      this.showURLImportStatus(
        '⏳ Fetching ingredient data from URL...',
        'info'
      );
      ...
      if (result.success) {
                      ...
                      Object.keys(data.nutritional_info || {}).length > 0
                        ...
                        <div class="mt-4">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                                ${Object.entries(data.nutritional_info)
                                  .map(
                                    ([key, value]) =>
        ...
        this.showURLImportStatus(
          `❌ Error: ${result.detail || 'Failed to fetch ingredient data'}`,
          'error'
        );
  ...
  async importFromURL() {
    ...
    try {
      ...
      if (result.success) {
        this.showURLImportStatus(
          `✅ Successfully imported "${result.ingredient.name}"!`,
          'success'
        );
        ...
        if (result.existing_ingredient) {
          this.showURLImportStatus(
            `⚠️ Ingredient "${result.existing_ingredient.name}" already exists in your library`,
            'warning'
          );
        } else {
          this.showURLImportStatus(
            `❌ Error: ${result.message || 'Failed to import ingredient'}`,
            'error'
          );
        }
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (
    document.getElementById('ingredient-library-container') ||
