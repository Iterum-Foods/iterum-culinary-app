...
class MultiRecipeDetector {
  ...
  detectRecipes(text) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        this.isRecipeBoundary(line, prevLine, nextLine, currentRecipe.length)
      ) {
        // Save current recipe if it has content
        if (currentRecipe.length > 10) {
          // Minimum lines for valid recipe
  ...
  parseRecipe(text, index) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (timeInfo.prepTime) {
        recipe.prepTime = timeInfo.prepTime;
      }
      if (timeInfo.cookTime) {
        recipe.cookTime = timeInfo.cookTime;
      }
      if (timeInfo.totalTime) {
        recipe.totalTime = timeInfo.totalTime;
      }
      ...
      if (section === 'ingredients') {
        ...
        section === 'title' &&
        !this.isIngredientsHeader(lowerLine) &&
        !this.isInstructionsHeader(lowerLine)
      ) {
  ...
  extractTime(line) {
    ...
    // Prep time
    const prepMatch = lowerLine.match(
      /prep(?:\s+time)?:?\s*(\d+)\s*(min|hour|hr)/i
    );
    if (prepMatch) {
      result.prepTime =
        parseInt(prepMatch[1]) * (prepMatch[2].startsWith('h') ? 60 : 1);
    }

    // Cook time
    const cookMatch = lowerLine.match(
      /cook(?:\s+time)?:?\s*(\d+)\s*(min|hour|hr)/i
    );
    if (cookMatch) {
      result.cookTime =
        parseInt(cookMatch[1]) * (cookMatch[2].startsWith('h') ? 60 : 1);
    }

    // Total time
    const totalMatch = lowerLine.match(
      /total(?:\s+time)?:?\s*(\d+)\s*(min|hour|hr)/i
    );
    if (totalMatch) {
      result.totalTime =
        parseInt(totalMatch[1]) * (totalMatch[2].startsWith('h') ? 60 : 1);
    }
  ...
  guessCategory(title, ingredients) {
    ...
    const categories = [
      {
        name: 'Appetizers',
        keywords: ['appetizer', 'starter', 'dip', 'spread', 'bruschetta']
      },
      { name: 'Salads', keywords: ['salad', 'slaw', 'greens'] },
      {
        name: 'Soups',
        keywords: ['soup', 'stew', 'chowder', 'bisque', 'broth']
      },
      ...
      {
        name: 'Breads',
        keywords: ['bread', 'roll', 'biscuit', 'muffin', 'scone']
      },
      {
        name: 'Breakfast',
        keywords: ['breakfast', 'pancake', 'waffle', 'omelet', 'eggs']
      }
    ];
  ...
  generateTags(recipe) {
    ...
    if (allText.includes('vegetarian')) {
    ...
    // Cooking methods
    if (allText.includes('grill')) {
      tags.push('grilled');
    }
    if (allText.includes('bake') || allText.includes('baked')) {
      tags.push('baked');
    }
    if (allText.includes('fry') || allText.includes('fried')) {
      tags.push('fried');
    }
    if (allText.includes('roast')) {
      tags.push('roasted');
    }
    if (allText.includes('slow cook')) {
    ...
    if (recipe.totalTime && recipe.totalTime >= 120) {
      tags.push('slow');
    }
