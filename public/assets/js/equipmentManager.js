...
if (typeof window.equipmentDatabase !== 'undefined') {
  ...
  async function loadEquipmentDatabase() {
    try {
      ...
      console.log(
        ...
        window.equipmentDatabase.length,
        'items'
      );
    } catch (error) {
  ...
  function setupEquipmentEventListeners() {
    ...
    const dbCategoryFilter = document.getElementById(
      'equipment-db-category-filter'
    );
    if (dbCategoryFilter) {
  ...
  function setupEquipmentManagementEventListeners() {
    ...
    if (cancelAddBtn) {
      cancelAddBtn.addEventListener('click', hideAddEquipmentModal);
    }
    if (closeEditModal) {
      closeEditModal.addEventListener('click', hideEditEquipmentModal);
    }
    if (cancelEditBtn) {
      cancelEditBtn.addEventListener('click', hideEditEquipmentModal);
    }
    if (closeDbModal) {
      closeDbModal.addEventListener('click', hideEquipmentDatabaseModal);
    }
    ...
    if (showMaintenanceBtn) {
      showMaintenanceBtn.addEventListener('click', () =>
        filterByMaintenance('due-soon')
      );
    }
    if (showRepairBtn) {
      showRepairBtn.addEventListener('click', () => filterByStatus('Repair'));
    }
    if (showOverdueBtn) {
      showOverdueBtn.addEventListener('click', () =>
        filterByMaintenance('overdue')
      );
    }
    if (resetFiltersBtn) {
    ...
    const closeWebstaurantModal = document.getElementById(
      'close-webstaurant-modal'
    );
    if (closeWebstaurantModal) {
      closeWebstaurantModal.addEventListener('click', closeWebstaurantModal);
    }

    const webstaurantSearchBtn = document.getElementById(
      'webstaurant-search-btn'
    );
    if (webstaurantSearchBtn) {
      webstaurantSearchBtn.addEventListener('click', () =>
        searchWebstaurantStore()
      );
    }

    const webstaurantSearchInput =
      document.getElementById('webstaurant-search');
    ...
    // Retry button
    const webstaurantRetryBtn = document.getElementById(
      'webstaurant-retry-btn'
    );
    if (webstaurantRetryBtn) {
      webstaurantRetryBtn.addEventListener('click', () =>
        searchWebstaurantStore()
      );
    }
  }
  ...
  function populateEditForm(equipment) {
    ...
    try {
      ...
      document.getElementById('edit-equipment-category').value =
        equipment.category;
      document.getElementById('edit-equipment-quantity').value =
        equipment.quantity || 1;
      document.getElementById('edit-equipment-brand').value =
        equipment.brand || '';
      document.getElementById('edit-equipment-serial').value =
        equipment.serialNumber || '';
      document.getElementById('edit-equipment-purchase-date').value =
        equipment.purchaseDate || '';
      document.getElementById('edit-equipment-warranty').value =
      ...
      document.getElementById('edit-equipment-location').value =
        equipment.location || '';
      document.getElementById('edit-equipment-last-maintenance').value =
        equipment.lastMaintenanceDate || '';
      document.getElementById('edit-equipment-next-maintenance').value =
        equipment.nextMaintenanceDate || '';
      document.getElementById('edit-equipment-maintenance-notes').value =
        equipment.maintenanceNotes || '';
      document.getElementById('edit-equipment-description').value =
  ...
  async function handleAddEquipment(event) {
    ...
    const equipment = {
      ...
      name:
        formData.get('equipment-name') ||
        document.getElementById('equipment-name').value,
      category:
        formData.get('equipment-category') ||
        document.getElementById('equipment-category').value,
      quantity:
        parseInt(
          formData.get('equipment-quantity') ||
            document.getElementById('equipment-quantity').value
        ) || 1,
      brand:
        formData.get('equipment-brand') ||
        document.getElementById('equipment-brand').value,
      serialNumber:
        formData.get('equipment-serial') ||
        document.getElementById('equipment-serial').value,
      purchaseDate:
        formData.get('equipment-purchase-date') ||
        document.getElementById('equipment-purchase-date').value,
      warrantyExpiry:
        formData.get('equipment-warranty') ||
        document.getElementById('equipment-warranty').value,
      status:
        formData.get('equipment-status') ||
        document.getElementById('equipment-status').value,
      location:
        formData.get('equipment-location') ||
        document.getElementById('equipment-location').value,
      lastMaintenanceDate:
        formData.get('equipment-last-maintenance') ||
        document.getElementById('equipment-last-maintenance').value,
      nextMaintenanceDate:
        formData.get('equipment-next-maintenance') ||
        document.getElementById('equipment-next-maintenance').value,
      maintenanceNotes:
        formData.get('equipment-maintenance-notes') ||
        document.getElementById('equipment-maintenance-notes').value,
      description:
        formData.get('equipment-description') ||
  ...
  async function handleEditEquipment(event) {
    ...
    const equipment = {
      ...
      quantity:
        parseInt(document.getElementById('edit-equipment-quantity').value) || 1,
      brand: document.getElementById('edit-equipment-brand').value,
      serialNumber: document.getElementById('edit-equipment-serial').value,
      purchaseDate: document.getElementById('edit-equipment-purchase-date')
        .value,
      ...
      lastMaintenanceDate: document.getElementById(
        'edit-equipment-last-maintenance'
      ).value,
      nextMaintenanceDate: document.getElementById(
        'edit-equipment-next-maintenance'
      ).value,
      maintenanceNotes: document.getElementById(
        'edit-equipment-maintenance-notes'
      ).value,
  ...
  function filterEquipment() {
    const searchTerm =
      document.getElementById('equipment-search')?.value.toLowerCase() || '';
    ...
    let filteredEquipment = window.currentUserEquipment.filter(equipment => {
      ...
      if (maintenanceFilter) {
        ...
        if (
          maintenanceFilter === 'due-month' &&
          maintenanceStatus !== 'due-month'
        ) {
          return false;
        }
      }
  ...
  function updateEquipmentSummaryStats() {
    ...
    if (repairCount) {
      repairCount.textContent = stats.repair;
    }
    if (totalCount) {
      totalCount.textContent = stats.total;
    }
    if (totalQuantityCount) {
      totalQuantityCount.textContent = stats.totalQuantity;
    }
  }
  ...
  function filterEquipmentDatabase() {
    const searchTerm =
      document.getElementById('equipment-db-search')?.value.toLowerCase() || '';
    const categoryFilter =
      document.getElementById('equipment-db-category-filter')?.value || '';
  ...
  function setupEquipmentFormEnhancements() {
    ...
    function addMaintenanceActivity(equipmentId, action, description = '') {
      const equipment = window.currentUserEquipment.find(
        e => e.id === equipmentId
      );
      if (!equipment) {
        return;
      ...
      let maintenanceLog = JSON.parse(
        localStorage.getItem('iterum_maintenance_log') || '[]'
      );
      maintenanceLog.push(maintenance);
      localStorage.setItem(
        'iterum_maintenance_log',
        JSON.stringify(maintenanceLog)
      );
      ...
      showNotification(
        `Maintenance activity logged for ${equipment.name}`,
        'success'
      );
    }

    // Auto-calculate next maintenance date based on last maintenance
    const lastMaintenanceInput = document.getElementById(
      'equipment-last-maintenance'
    );
    const nextMaintenanceInput = document.getElementById(
      'equipment-next-maintenance'
    );
  ...
  async function updateEquipmentInUser(equipment) {
    try {
      ...
      if (result.success) {
        const index = window.currentUserEquipment.findIndex(
          e => e.id === equipment.id
        );
        if (index !== -1) {
  ...
  function loadCurrentUserEquipment() {
    ...
    if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
      ...
      const savedEquipment = localStorage.getItem(
        `equipment_${currentUser.id}`
      );
      window.currentUserEquipment = savedEquipment
  ...
  async function deleteEquipment(equipmentId) {
    ...
    try {
      // Remove from array
      window.currentUserEquipment = window.currentUserEquipment.filter(
        e => e.id !== equipmentId
      );
      ...
      localStorage.setItem(
        `equipment_${currentUser.id}`,
        JSON.stringify(window.currentUserEquipment)
      );
  ...
  document
    .getElementById('bulk-import-equipment-pdf')
    ?.addEventListener('change', async function (e) {
      ...
      mappingDiv.innerHTML = headers
        .map(
          (h, i) =>
            `<select class='notebook-input text-xs' data-col='${i}'>${fields.map(f => `<option value='${f}'>${f}</option>`).join('')}</select>`
        )
        .join('');
      head.innerHTML =
      ...
      if (!document.getElementById('equipment-pdf-import-btn')) {
        ...
        btn.addEventListener('click', function () {
          ...
          showNotification(
            'Imported ' + imported.length + ' equipment items from PDF.'
          );
          previewDiv.classList.add('hidden');
        });
      }
  ...
  function isRefrigerationEquipment(equipment) {
    ...
    if (
      equipmentCategory.includes('refrigeration') ||
      equipmentCategory.includes('storage')
    ) {
      return true;
    }
  ...
  async function setupHACCPForRefrigeration(equipment) {
    try {
      ...
      if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
        const existingHACCP =
          window.userDataManager.getUserData('haccp_tasks') || [];
        ...
        const currentUser = getCurrentUser();
        if (currentUser) {
          const existingHACCP = JSON.parse(
            localStorage.getItem(`haccp_tasks_${currentUser.id}`) || '[]'
          );
          existingHACCP.push(...haccpTasks);
          localStorage.setItem(
            `haccp_tasks_${currentUser.id}`,
            JSON.stringify(existingHACCP)
          );
        }
      }
      ...
      showNotification(
        `✅ HACCP temperature monitoring setup for ${equipment.name}! 3x daily temperature checks and daily notes will be tracked.`,
        'success'
      );
      ...
      setTimeout(() => {
        showNotification(
          `📋 Created ${haccpTasks.length} HACCP tasks for the next 30 days. Check the HACCP page to view and complete tasks.`,
          'info'
        );
      }, 2000);

      console.log(
        'HACCP tasks created for refrigeration equipment:',
        equipment.name,
        haccpTasks
      );
    } catch (error) {
      console.error(
        'Error setting up HACCP for refrigeration equipment:',
        error
      );
      showNotification(
        'Warning: Could not setup HACCP monitoring for refrigeration equipment.',
        'warning'
      );
    }
  }
      ...
      if (file) {
        try {
          ...
          if (data.equipment && Array.isArray(data.equipment)) {
            ...
            showNotification(
              `Imported ${data.equipment.length} equipment items successfully!`,
              'success'
            );
            loadCurrentUserEquipment();
          } else {
  ...
  async function parseEquipmentUrl() {
    ...
    try {
      ...
      const response = await fetch(
        'http://localhost:8000/api/integrations/equipment/parse-url',
        {
          method: 'POST',
          headers: {
  ...
  function displayParsedEquipment(equipment) {
    ...
    document.getElementById('parsed-brand').textContent =
      equipment.brand || 'Unknown';
    document.getElementById('parsed-model').textContent =
      equipment.model || 'N/A';
    document.getElementById('parsed-category').textContent =
      equipment.category || 'Equipment';
    document.getElementById('parsed-price').textContent =
      equipment.price || '0.00';
    document.getElementById('parsed-supplier').textContent =
      equipment.supplier || 'Unknown';
    document.getElementById('parsed-description').textContent =
    ...
    // Show preview
    document
      .getElementById('parsed-equipment-preview')
  ...
  function importParsedEquipment() {
    ...
    showNotification(
      `Equipment "${equipment.name}" imported from ${equipment.supplier}! Review and save.`,
      'success'
    );
  }
  ...
  function saveRecentUrl(url, supplier) {
    const recent = JSON.parse(
      localStorage.getItem('recentEquipmentUrls') || '[]'
    );
    const urlData = {
      url: url,
      supplier: supplier,
  ...
  async function searchWebstaurantStore(query = null) {
    ...
    try {
      // Call the backend API
      const response = await fetch(
        `http://localhost:8000/api/integrations/webstaurant/search?query=${encodeURIComponent(searchTerm)}&limit=20`
      );
  ...
  async function importWebstaurantProduct(productId) {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/api/integrations/webstaurant/product/${productId}`
      );
      ...
      showNotification(
        `✅ Imported ${product.name} from WebstaurantStore`,
        'success'
      );
    } catch (error) {
