...
class StatePersistenceManager {
  ...
  async init() {
    ...
    try {
      ...
      console.error(
        '❌ State Persistence Manager initialization failed:',
        error
      );
    }
  }
  ...
  setupCrossTabSync() {
    ...
    window.addEventListener('storage', event => {
      ...
      if (
        event.key?.startsWith(this.KEYS.PROJECT_USER) ||
        event.key === this.KEYS.PROJECT
      ) {
  ...
  async verifyState() {
    try {
      ...
      if (userJson && userId) {
        ...
        if (
          window.authManager &&
          window.authManager.currentUser?.userId !== userId
        ) {
          window.authManager.currentUser = user;
        }
      }
  ...
  async syncAllPageElements() {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent('stateSyncComplete', {
          detail: {
            user: window.currentUser,
  ...
  async syncUserDisplayElements() {
    ...
    const userNameElements = document.querySelectorAll(
      '[id*="user-name"], [data-user-name], #current-user, #dropdown-user-name'
    );
    ...
    const avatarElements = document.querySelectorAll(
      '#user-avatar-initial, [data-user-avatar]'
    );
  ...
  async syncProjectDisplayElements() {
    ...
    const projectNameElements = document.querySelectorAll(
      '[data-project-name], .project-selector-name'
    );
    ...
    const projectIconElements = document.querySelectorAll(
    ...
    const projectSelectors = document.querySelectorAll(
      '#header-project-selector, #mobile-project-selector, [data-project-selector]'
    );
  ...
  updateHeaderElements() {
    ...
    if (
      ...
      typeof window.projectManager.updateProjectUI === 'function'
    ) {
