...
class EquipmentManager {
  ...
  unlinkServingWareFromRecipe(equipmentId, recipeId) {
    ...
    item.linkedRecipes = item.linkedRecipes.filter(
  ...
  addEquipment(equipmentData, options = {}) {
    ...
    const equipment = {
      ...
      currentValue:
        parseFloat(equipmentData.currentValue) ||
        parseFloat(equipmentData.purchasePrice) ||
        0,
    ...
    if (!options.skipDuplicateCheck) {
      const duplicate = this.equipment.find(
        item => item.name.toLowerCase() === equipment.name.toLowerCase()
      );
      if (duplicate) {
  ...
  getEquipmentForRecipe(recipeId) {
    ...
    if (!recipe || !recipe.equipmentNeeded) {
      return [];
    }

    return recipe.equipmentNeeded
      .map(equipId => this.getById(equipId))
      .filter(Boolean);
  }
  ...
  updateInventory(equipmentId, change, reason = '') {
    ...
    console.log(
      `📦 Inventory updated: ${equipment.name} (${equipment.inStock}/${equipment.quantity})`
    );
    return true;
  }
  ...
  getStatistics() {
    ...
    this.equipment.forEach(equip => {
      // By category
      stats.byCategory[equip.category] =
        (stats.byCategory[equip.category] || 0) + 1;
  ...
  linkEquipmentToRecipe(recipeId, equipmentIds) {
    ...
    recipe.equipmentDetails = equipmentIds
      .map(id => {
    ...
    console.log(
      `🔗 Linked ${equipmentIds.length} equipment items to recipe:`,
      recipe.title
    );
    return true;
  }
  ...
  reserveEquipment(equipmentId, quantity, reservedFor = '') {
    ...
    console.log(
      `📋 Reserved ${quantity}x ${equipment.name} for ${reservedFor}`
    );
    return reservation;
  }
