...
class AuthManager {
  ...
  async loadSession() {
    ...
    try {
      const sessionActive = localStorage.getItem(
  ...
  async signUpWithEmail(name, email, password) {
    ...
    try {
      ...
      const firebaseUser = await firebaseAuth.createUserWithEmail(
        email,
        password,
        name
      );
  ...
  async syncWithBackend(user, idToken) {
    ...
    try {
      const response = await fetch(
        `${this.API_BASE_URL}/api/firebase/sync-user`,
        {
          method: 'POST',
          headers: {
      ...
      localStorage.setItem(
        this.STORAGE_KEYS.BACKEND_USER,
  ...
  async saveSession(user) {
    ...
    try {
      ...
      this.saveToFirestore(user).catch(e =>
        this.log('⚠️ Firestore save failed (non-critical):', e)
      );
      ...
      setTimeout(async () => {
        try {
          ...
          window.dispatchEvent(
            new CustomEvent('initialDataLoaded', {
  ...
  async waitForFirebaseAuth(maxAttempts = 50) {
        ...
        if (i > 10) {
          ...
          try {
            if (
              ...
              window.firebaseAuth.isInitialized !== false
            ) {
    ...
    if (window.firebaseAuth) {
      this.log(
        '⚠️ Firebase Auth object exists but not marked as initialized, attempting to use anyway...'
      );
      return window.firebaseAuth;
    }

    throw new Error(
      'Firebase Auth not available after ' + maxAttempts * 100 + 'ms'
    );
  }
  ...
  handleStorageChange(event) {
    if (
      event.key === this.STORAGE_KEYS.SESSION_ACTIVE ||
  ...
  getAuthHeaders() {
    const token =
      this.firebaseToken ||
      localStorage.getItem(this.STORAGE_KEYS.FIREBASE_TOKEN);
    const headers = {
      'Content-Type': 'application/json'
    };
  ...
  getDiagnostics() {
    return {
      ...
      localStorage: {
        ...
        current_user_exists: !!localStorage.getItem(
          this.STORAGE_KEYS.CURRENT_USER
        ),
        last_login: localStorage.getItem(this.STORAGE_KEYS.LAST_LOGIN),
        backend_user_exists: !!localStorage.getItem(
          this.STORAGE_KEYS.BACKEND_USER
        ),
        firebase_token_exists: !!localStorage.getItem(
          this.STORAGE_KEYS.FIREBASE_TOKEN
        )
      },
      firebaseAuth: {
