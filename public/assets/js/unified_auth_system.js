...
class UnifiedAuthSystem {
  ...
  async init() {
    ...
    try {
      ...
      if (!hasSession) {
        console.log(
          '🚫 NO ACCESS: User not authenticated, showing mandatory auth'
        );
  ...
  async waitForEnhancedUserSystem() {
    ...
    console.log(
      '⚠️ Enhanced user system not available after waiting, proceeding anyway'
    );
  }
  ...
  async checkExistingSession() {
    ...
    if (window.firebaseAuth && window.firebaseAuth.isSignedIn()) {
      ...
      if (firebaseUser) {
        console.log(
          '🔥 Firebase session found, restoring user:',
          firebaseUser.email
        );
    ...
    if (window.securityUtils) {
      sessionActive = await window.securityUtils.safeLocalStorage.getItem(
        'session_active',
        false
      );
      currentUser = await window.securityUtils.safeLocalStorage.getItem(
        'current_user',
        true
      );
    } else {
  ...
  async forceUserSelection() {
    ...
    console.log(
      '🔍 Debug - window.unifiedAuthSystem:',
    ...
    console.log(
      `📊 After loading, savedUsers count: ${this.savedUsers.length}`
    );
    ...
    if (this.savedUsers.length > 0) {
      ...
      try {
        ...
        if (finalImport && finalImport.length > 0) {
          console.log(
            `✅ Final import successful: ${finalImport.length} users`
          );
  ...
  async loadSavedUsers() {
    try {
      ...
      if (
        window.enhancedUserSystem &&
        typeof window.enhancedUserSystem.loadUsersFromFiles === 'function'
      ) {
        try {
          console.log(
            '🔍 Enhanced user system available, loading users from files...'
          );
          ...
          if (
            ...
            window.enhancedUserSystem.users.length > 0
          ) {
            this.savedUsers = [...window.enhancedUserSystem.users];
            console.log(
              `✅ Loaded ${this.savedUsers.length} users from enhanced user system`
            );

            // Also save to localStorage for consistency
            localStorage.setItem(
              'saved_users',
              JSON.stringify(this.savedUsers)
            );
          ...
          console.warn(
            '⚠️ Failed to load from enhanced user system:',
            enhancedError
          );
        }
      }
      ...
      if (this.isOnline) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.log(
              `✅ Loaded ${this.savedUsers.length} users from backend`
            );
          } else {
          ...
          console.warn(
            '⚠️ Failed to load users from backend, using local storage:',
            error
          );
      ...
      if (this.savedUsers.length === 0) {
        console.log(
          '⚠️ No users found, attempting to import from JSON file...'
        );
        try {
          const importedUsers = await this.importUsersFromJSON();
          if (importedUsers && importedUsers.length > 0) {
            console.log(
              `✅ Successfully imported ${importedUsers.length} users from JSON file`
            );
  ...
  loadSavedUsersFromLocal() {
    try {
      ...
      if (savedUsersData) {
        ...
        console.log(
          `✅ Loaded ${this.savedUsers.length} users from local storage:`,
          this.savedUsers
        );
      } else {
        this.savedUsers = [];
  ...
  ensureAllUsersLoaded() {
    try {
      ...
      if (savedUsersData) {
        ...
        console.log(
          `📦 Found ${localStorageUsers.length} users in localStorage`
        );
        ...
        if (this.savedUsers.length === 0) {
          ...
          this.importUsersFromJSON()
            .then(importedUsers => {
              if (importedUsers && importedUsers.length > 0) {
                console.log(
                  `✅ Imported ${importedUsers.length} users from JSON in ensureAllUsersLoaded`
                );
                this.savedUsers = importedUsers;
                // Update localStorage
                localStorage.setItem(
            ...
            .catch(error => {
              console.warn(
                '⚠️ JSON import failed in ensureAllUsersLoaded:',
                error
              );
            });
        }
  ...
  checkAlternativeUserStorage() {
    try {
      ...
      console.log(
        `🔍 Checking ${userKeys.length} potential user storage keys...`
      );
  ...
  syncWithEnhancedUserSystem() {
    try {
      if (window.enhancedUserSystem && window.enhancedUserSystem.users) {
        ...
        console.log(
          `📦 Found ${enhancedUsers.length} users in enhanced user system`
        );
        ...
        enhancedUsers.forEach(enhancedUser => {
          if (!this.savedUsers.find(u => u.id === enhancedUser.id)) {
            ...
            console.log(
              `➕ Added user from enhanced system: ${enhancedUser.name}`
            );
          }
        });
  ...
  showUserSelectionModal() {
    ...
    modal.className =
      'auth-modal-overlay fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] opacity-0 transition-opacity duration-300';
    ...
    if (form) {
      form.addEventListener('submit', async e => {
        ...
        const profileData = {
          ...
          restaurant: document
            .getElementById('profile-restaurant')
            .value.trim(),
    ...
    if (form) {
      form.addEventListener('submit', async e => {
        ...
        try {
          ...
          alert(
            'Online login will be implemented in a future update. Please use offline profile creation for now.'
          );
          this.removeExistingModals();
          this.showCreateProfileModal();
        } catch (error) {
  ...
  removeExistingModals() {
    const existingModals = document.querySelectorAll(
      '.auth-modal-overlay, .mandatory-auth-overlay'
    );
  ...
  showUserSelection() {
    ...
    const modal = document.createElement('div');
    modal.className =
      'auth-modal-overlay fixed inset-0 bg-gradient-to-br from-red-900 via-purple-900 to-blue-900 bg-opacity-90 flex items-center justify-center z-[9999] opacity-0 transition-opacity duration-500';
    modal.style.zIndex = '9999';
  ...
  showMandatoryAuthentication() {
    ...
    const modal = document.createElement('div');
    modal.className =
      'mandatory-auth-overlay fixed inset-0 bg-gray-900 flex items-center justify-center z-[99999]';
  ...
  hideAllContent() {
    ...
    const contentSections = document.querySelectorAll(
      '.content, .app-content, .main-content, .page-content, .landing-hero, .features-section, .cta-section'
    );
  ...
  showUserSwitchDropdown() {
    ...
    if (!headerUser) {
      console.warn(
        '⚠️ Header user section not found, falling back to full-screen overlay'
      );
                            ...
                            <div class="flex items-center space-x-2">
                                ${
                                  user.id === this.currentUser?.id
                                    ...
                                    : '<button onclick="unifiedAuth.selectUser(\'' +
                                      user.id +
                                      '\')" class="bg-purple-500 hover:bg-purple-600 text-white px-3 py-1 rounded text-xs">Switch To</button>'
                                }
                            </div>
                        </div>
                    `
                                ...
                                <div class="flex items-center space-x-2">
                                    ${
                                      user.id === this.currentUser?.id
                                        ...
                                        : '<button onclick="unifiedAuth.selectUser(\'' +
                                          user.id +
                                          '\')" class="bg-purple-500 hover:bg-purple-600 text-white px-4 py-2 rounded-lg text-sm">Switch To</button>'
                                    }
                                </div>
                            </div>
                        `
  ...
  async selectUser(userId) {
    try {
      ...
      if (window.securityUtils) {
        await window.securityUtils.safeLocalStorage.setItem(
          'current_user',
          user,
          true
        );
  ...
  showSuccessMessage(message) {
    ...
    successDiv.className =
      'fixed top-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-[10000] transform translate-x-full transition-transform duration-300';
  ...
  async loadBasicUserData(user) {
    try {
      if (
        ...
        typeof window.UserDataManager === 'function'
      ) {
    ...
    try {
      ...
      if (this.isOnline) {
        const response = await fetch(
          'http://localhost:8000/api/profiles/offline',
          {
            method: 'POST',
        ...
        if (!response.ok) {
          throw new Error('Failed to create offline profile');
        }
        const data = await response.json();
        user = data.user;
      } else {
      ...
      if (window.initializeProjectsForNewUser) {
        try {
          const projectInitSuccess = window.initializeProjectsForNewUser(
            user.id
          );
          if (projectInitSuccess) {
            ...
            console.warn(
              '⚠️ Failed to initialize projects for new user:',
              user.id
            );
          }
        } catch (error) {
    ...
    try {
      ...
      if (this.isOnline) {
        ...
        const loginData = {
          email:
            profileData.email ||
            `${profileData.name.toLowerCase().replace(/\s+/g, '')}@local.com`,
          password: 'default123', // Default password since we're username-only
          name: profileData.name
        };

        const response = await fetch(
          'http://localhost:8000/api/profiles/login',
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  updateAuthUI(user) {
    ...
    if (userInfo) {
      userInfo.textContent = user && user.name ? user.name : 'Guest';
    }
    if (currentUserSpan) {
      currentUserSpan.textContent = user && user.name ? user.name : '';
    }
    ...
    if (userRestaurant) {
      userRestaurant.textContent = user.restaurant || '';
    }
    ...
    if (userAvatar) {
      userAvatar.src =
        user.photoURL ||
        `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><circle cx="50" cy="50" r="50" fill="%234285F4"/><text x="50" y="65" font-family="Arial" font-size="40" fill="white" text-anchor="middle">${user.name.charAt(0).toUpperCase()}</text></svg>`;
    }
  ...
  async handleLogin(loginData) {
    ...
    try {
      ...
      if (this.isOnline) {
        ...
        const savedUser = this.savedUsers.find(
          u => u.email === loginData.email || u.username === loginData.email
        );
        ...
        if (
          savedUser.password &&
          savedUser.password !== 'localdev123' &&
  ...
  updateAuthUI() {
    try {
      if (this.currentUser) {
        console.log(
          '🔄 Updating authentication UI for user:',
        ...
        window.dispatchEvent(
          new CustomEvent('showUserToggle', {
...
- Check console for detailed information
  ...
  async importUsersFromJSON(isManualImport = false) {
    ...
    try {
      ...
      console.log(
        `📦 Loaded ${users.length} users from JSON file (${accessiblePath}):`,
        users
      );
      ...
      console.log(
        `✅ Successfully imported users. Total users: ${this.savedUsers.length}`
      );
      ...
      if (!isManualImport && this.savedUsers.length > 0) {
        console.log(
          '🔄 Refreshing user selection interface after automatic import...'
        );
...
window.reloadUsers = async function () {
  if (window.unifiedAuth) {
    ...
    console.log(
      '✅ Users reloaded. Count:',
      window.unifiedAuth.savedUsers.length
    );
