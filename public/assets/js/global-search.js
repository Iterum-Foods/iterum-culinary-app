(function () {
  if (window.iterumGlobalSearchInitialized) {
    return;
  }
  window.iterumGlobalSearchInitialized = true;
  ...
  backdrop.className = 'it-gs-backdrop';
  const modal = document.createElement('div');
  modal.className = 'it-gs';
  modal.innerHTML = `
  ...
  function getData() {
    ...
    return {
      ...
      ingredients:
        safeParse('ingredients') || safeParse('iterum:ingredients') || [],
  ...
  function render(query) {
    ...
    let html = '';
    if (r.length) {
      html += `<div class="it-gs-group"><div class="it-gs-title">Recipes</div>${r.map(x => `<div class="it-gs-item" data-link="recipe-library.html#id=${x.id || x.uuid || encodeURIComponent(x.name)}"><span>${x.name || x.title}</span><span class="it-gs-tag">open</span></div>`).join('')}</div>`;
    }
    if (i.length) {
      html += `<div class="it-gs-group"><div class="it-gs-title">Ingredients</div>${i.map(x => `<div class="it-gs-item" data-link="ingredients.html#q=${encodeURIComponent(x.name)}"><span>${x.name}</span><span class="it-gs-tag">view</span></div>`).join('')}</div>`;
    }
    if (v.length) {
      html += `<div class="it-gs-group"><div class="it-gs-title">Vendors</div>${v.map(x => `<div class="it-gs-item" data-link="vendor-management.html#q=${encodeURIComponent(x.name)}"><span>${x.name}</span><span class="it-gs-tag">open</span></div>`).join('')}</div>`;
    }
    if (!html) {
      html = `<div class="it-gs-group"><div class="it-gs-item"><span>No results</span></div></div>`;
    }
