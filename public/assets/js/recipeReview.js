...
class RecipeReviewManager {
  ...
  setupEventListeners() {
    ...
    if (pendingReviewsBtn) {
      pendingReviewsBtn.addEventListener('click', () =>
    ...
    const approveSelectedBtn = document.getElementById(
      'approve-selected-recipes'
    );
    const rejectSelectedBtn = document.getElementById(
      'reject-selected-recipes'
    );

    if (approveAllBtn) {
      approveAllBtn.addEventListener('click', () =>
        this.batchReview('approve', 'all')
      );
    }
    if (rejectAllBtn) {
      rejectAllBtn.addEventListener('click', () =>
        this.batchReview('reject', 'all')
      );
    }
    if (approveSelectedBtn) {
      approveSelectedBtn.addEventListener('click', () =>
        this.batchReview('approve', 'selected')
      );
    }
    if (rejectSelectedBtn) {
      rejectSelectedBtn.addEventListener('click', () =>
        this.batchReview('reject', 'selected')
      );
    }
    ...
    if (selectAllBtn) {
      selectAllBtn.addEventListener('click', () => this.selectAllRecipes());
    }
    ...
    if (openLocationBtn) {
      openLocationBtn.addEventListener('click', () => this.openFileLocation());
    }
    ...
    if (saveEditsBtn) {
      saveEditsBtn.addEventListener('click', () => this.saveEdits());
    }
    if (cancelEditsBtn) {
      cancelEditsBtn.addEventListener('click', () => this.cancelEdits());
    }
    ...
    if (addInstructionBtn) {
      addInstructionBtn.addEventListener('click', () =>
        this.addEditInstruction()
      );
    }
    if (addTagBtn) {
      addTagBtn.addEventListener('click', () => this.addEditTag());
    }
  ...
  async loadPendingReviews() {
    try {
      ...
      let pendingLocal = [];
      try {
        pendingLocal = JSON.parse(
          localStorage.getItem('pendingRecipes') || '[]'
        ).map(r => ({ ...r, _source: 'Uploaded' }));
      } catch (e) {
      ...
      try {
        ...
        const response = await fetch(
          `/api/recipes/review/pending?page=${this.currentPage}&limit=${this.itemsPerPage}`,
          {
            method: 'GET',
        ...
        if (response.ok) {
          ...
          if (response.status === 404) {
            console.info(
              '📝 Recipe review endpoint not implemented yet - using local storage only'
            );
        ...
        if (e.name === 'AbortError') {
          ...
          console.warn(
            '🔌 Cannot connect to backend server - using offline mode'
          );
        } else {
      ...
      try {
        const fallbackLocal = JSON.parse(
          localStorage.getItem('pendingRecipes') || '[]'
        );
        this.pendingRecipes = fallbackLocal.map(r => ({
          ...r,
          _source: 'Uploaded'
        }));
        this.displayPendingReviews();
        ...
        this.showUserMessage(
          'Using offline mode - only locally stored recipes available',
          'warning'
        );
      } catch (fallbackError) {
        ...
        this.displayPendingReviews();
        this.showUserMessage(
          'Unable to load recipes. Please refresh the page.',
          'error'
        );
      }
    }
    ...
    if (fileType === 'text' || fileType === 'txt') {
      ...
      const tableRows = lines
        .map(line => {
          const cells = line
            .split(',')
            .map(cell => `<td class="px-2 py-1 border">${cell.trim()}</td>`);
  ...
  clearFilePreview() {
    ...
    if (fileInfo) {
      fileInfo.textContent = 'Select a recipe to view the original file';
    }
    if (downloadBtn) {
  ...
  async downloadOriginalFile() {
    ...
    try {
      const response = await fetch(
        `/api/recipes/${this.currentFilePreview.id}/download-original`,
        {
          method: 'GET'
        }
      );
  ...
  async reviewRecipe(recipeId, action) {
    ...
    if (action === 'approve' || action === 'reject') {
      notes =
        prompt(`Enter review notes for ${action}ing this recipe (optional):`) ||
        '';
    }
    try {
      if (source === 'Uploaded') {
        ...
        if (action === 'approve') {
          const payload = {
            ...
            ingredients: (recipe.ingredients || []).map(i =>
              typeof i === 'string' ? { name: i, amount: 0, unit: '' } : i
            ),
            instructions: (recipe.instructions || recipe.steps || []).map(i =>
          ...
          if (res.ok) {
            ...
            this.showNotification(
              'Recipe approved and saved to database',
              'success'
            );
          } else {
            this.showNotification(
          ...
          localStorage.setItem('pendingRecipes', JSON.stringify(pending));
          this.showNotification(
            'Recipe rejected and removed from uploads',
            'success'
          );
        }
      } else {
  ...
  async batchReview(action, scope) {
    ...
    const notes = prompt(
      `Enter review notes for ${action}ing ${recipeIds.length} recipes (optional):`
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.pendingRecipes = this.pendingRecipes.filter(
          r => !recipeIds.includes(r.id)
        );
        this.selectedRecipes.clear();
        ...
        if (
          ...
          recipeIds.includes(this.currentFilePreview.id)
        ) {
          this.clearFilePreview();
        }
  ...
  updateEditModeUI() {
    ...
    if (this.editMode) {
      if (toggleBtn) {
        toggleBtn.textContent = '👁️ View File';
      }
      if (saveBtn) {
      ...
      if (toggleBtn) {
        toggleBtn.textContent = '✏️ Quick Edit';
      }
      if (saveBtn) {
  ...
  populateEditForm() {
    ...
    document.getElementById('edit-recipe-category').value =
      recipe.category || 'Imported';
    document.getElementById('edit-recipe-cuisine').value =
      recipe.cuisine_type || 'Unknown';
    document.getElementById('edit-recipe-servings').value =
      recipe.servings || 4;
    document.getElementById('edit-recipe-difficulty').value =
      recipe.difficulty_level || 'Medium';
    document.getElementById('edit-recipe-prep-time').value =
      recipe.prep_time || 0;
    document.getElementById('edit-recipe-cook-time').value =
      recipe.cook_time || 0;
  ...
  populateTags(tags) {
    ...
    tags.forEach(tag => {
      ...
      tagSpan.className =
        'bg-orange-200 text-orange-800 px-2 py-1 rounded text-sm flex items-center gap-1';
      tagSpan.innerHTML = `
                ${tag}
  ...
  async saveEdits() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/recipes/${this.currentFilePreview.id}`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
  ...
  collectEditFormData() {
    ...
    // Collect ingredients
    document
      .querySelectorAll('#edit-ingredients-container .ingredient-name')
      .forEach((nameInput, index) => {
        const amountInput =
          nameInput.parentElement.querySelector('.ingredient-amount');
        const unitSelect =
          nameInput.parentElement.querySelector('.ingredient-unit');
    ...
    document
      .querySelectorAll('#edit-instructions-container .instruction-text')
    ...
    return {
      ...
      servings:
        parseInt(document.getElementById('edit-recipe-servings').value) || 4,
      difficulty_level: document.getElementById('edit-recipe-difficulty').value,
      prep_time:
        parseInt(document.getElementById('edit-recipe-prep-time').value) || 0,
      cook_time:
        parseInt(document.getElementById('edit-recipe-cook-time').value) || 0,
  ...
  updateRecipeCard(recipe) {
    ...
    recipeCards.forEach(card => {
      ...
      if (recipeId == recipe.id) {
        ...
        if (categoryElement) {
          categoryElement.textContent = recipe.category || 'Uncategorized';
        }

        const cuisineElement = card.querySelector('.cuisine');
        if (cuisineElement) {
          cuisineElement.textContent = recipe.cuisine_type || 'Unknown';
        }
      }
    });
  }
