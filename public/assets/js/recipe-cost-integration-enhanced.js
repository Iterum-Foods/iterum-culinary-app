...
class RecipeCostIntegrationEnhanced {
  ...
  async calculateIngredientCostForPrep(name, quantity, unit) {
    ...
    if (this.costCalculator) {
      const costData = this.costCalculator.calculateIngredientCost(
        name,
        quantity,
        unit
      );
  ...
  async calculateMenuItemCost(menuItem) {
    ...
    const plateCost = parseFloat(
      recipeCost.costPerServing || recipeCost.costPerYieldUnit || 0
    );
