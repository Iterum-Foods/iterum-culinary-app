  ...
  class HACCPManager {
    ...
    loadFromLocalStorage() {
      ...
      if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
        this.temperatureReadings =
          window.userDataManager.getUserHACCPReadings();
        this.sanitizerTests =
          window.userDataManager.getUserData('haccp_sanitizer') || [];
        this.dishwasherTests =
          window.userDataManager.getUserData('haccp_dishwasher') || [];
      } else {
    ...
    saveToLocalStorage() {
      ...
      if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
        ...
        localStorage.setItem(
          ...
          JSON.stringify(this.temperatureReadings)
        );
        localStorage.setItem(
          'iterum_haccp_sanitizer',
          JSON.stringify(this.sanitizerTests)
        );
        localStorage.setItem(
          'iterum_haccp_dishwasher',
          JSON.stringify(this.dishwasherTests)
        );
      }
    }
    ...
    loadRefrigeratorsFromEquipment() {
      ...
      if (equipmentData) {
        ...
        this.refrigerators = equipment.filter(
          item =>
    ...
    setupEventListeners() {
      ...
      if (quickAddBtn) {
        quickAddBtn.addEventListener('click', () =>
          this.addQuickTemperatureReading()
        );
      }
      ...
      if (addSanitizerBtn) {
        addSanitizerBtn.addEventListener('click', () =>
          this.addSanitizerTest()
        );
      }
      ...
      if (addDishwasherBtn) {
        addDishwasherBtn.addEventListener('click', () =>
          this.addDishwasherTest()
        );
      }
      ...
      if (filterFridge) {
        filterFridge.addEventListener('change', () =>
    ...
    addQuickTemperatureReading() {
      ...
      if (!fridgeId || isNaN(temperature)) {
        this.showNotification(
          'Please select a refrigerator and enter a valid temperature',
          'error'
        );
        return;
      }
      ...
      this.showNotification(
        `Temperature logged: ${fridge.name} - ${temperature}°F`,
        'success'
      );
    }
    ...
    showAddReadingModal() {
      ...
      addBtn.addEventListener('click', () =>
        this.addModalTemperatureReading(modal)
      );
    ...
    getTemperatureStatus(temperature, category) {
      const ranges =
        this.temperatureRanges[category] || this.temperatureRanges.refrigerator;
    ...
    updateTodayReadings() {
      ...
      const todayReadings = this.temperatureReadings.filter(
        reading => new Date(reading.timestamp).toDateString() === today
      );

      if (todayReadings.length === 0) {
        todayContainer.innerHTML =
          '<p class="text-gray-500 col-span-full text-center py-8">No temperature readings for today</p>';
        return;
      }

      todayContainer.innerHTML = todayReadings
        .map(
          reading => `
    ...
    updateHistoricalDisplay(tabType = 'temperature') {
      ...
      const filterFridge = document.getElementById(
        'haccp-filter-fridge'
      )?.value;
      ...
      if (filterDate && filterDate !== 'all') {
        ...
        filteredData = filteredData.filter(
          item => new Date(item.timestamp) >= daysAgo
        );
      }
    ...
    updateStatus() {
      ...
      const alerts = todayReadings.filter(
    ...
    checkForAlerts() {
      ...
      const todaySanitizerTests = this.sanitizerTests.filter(
        test => new Date(test.timestamp).toDateString() === today
      );
      const todayDishwasherTests = this.dishwasherTests.filter(
        test => new Date(test.timestamp).toDateString() === today
      );

      const tempAlerts = todayReadings.filter(
        reading => reading.status !== 'normal'
      );
      const sanitizerAlerts = todaySanitizerTests.filter(
        test => test.status !== 'normal'
      );
      const dishwasherAlerts = todayDishwasherTests.filter(
    ...
    updateTodaySanitizerTests() {
      const today = new Date().toDateString();
      const todayTests = this.sanitizerTests.filter(
        test => new Date(test.timestamp).toDateString() === today
      );
      ...
      if (todayTests.length > 0) {
        const sanitizerCards = todayTests
          .map(
            test => `
                    <div class="bg-white border border-gray-200 rounded-lg p-4">
                        <div class="flex justify-between items-start mb-2">
    ...
    updateTodayDishwasherTests() {
      const today = new Date().toDateString();
      const todayTests = this.dishwasherTests.filter(
        test => new Date(test.timestamp).toDateString() === today
      );
      ...
      if (todayTests.length > 0) {
        const dishwasherCards = todayTests
          .map(
            test => `
                    <div class="bg-white border border-gray-200 rounded-lg p-4">
                        <div class="flex justify-between items-start mb-2">
    ...
    editReading(readingId) {
      ...
      saveBtn.addEventListener('click', () =>
        this.saveEditedReading(readingId, modal)
      );
    ...
    saveEditedReading(readingId, modal) {
      ...
      reading.status = this.getTemperatureStatus(
        reading.temperature,
        fridge.category || 'refrigerator'
      );
    ...
    deleteReading(readingId) {
      if (
        !confirm('Are you sure you want to delete this temperature reading?')
      ) {
        return;
      }

      this.temperatureReadings = this.temperatureReadings.filter(
        r => r.id !== readingId
      );
      this.saveToLocalStorage();
      this.updateDisplay();
    ...
    generateHACCPReport() {
      ...
      const weekReadings = this.temperatureReadings.filter(
      ...
      Object.values(report.refrigerators).forEach(fridge => {
        ...
        fridge.compliance_rate = (
          ((fridge.readings - fridge.alerts) / fridge.readings) *
          100
        ).toFixed(1);
      });
      ...
      report.compliance_rate =
        weekReadings.length > 0
          ? (
              ((weekReadings.length - report.alerts.length) /
                weekReadings.length) *
              100
            ).toFixed(1)
          : 0;
                ...
                <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
                    ...
                    <div class="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
                        ...
                        <div class="space-y-4">
                            ...
                            ${Object.entries(report.refrigerators)
                              .map(
                                ([name, data]) => `
    ...
    addTemperatureFromDailyNotes(
      ...
      const fridge = this.refrigerators.find(
        f =>
    ...
    addSanitizerTest() {
      ...
      if (!location || !type || isNaN(concentration)) {
        this.showNotification(
          'Please fill in all sanitizer test fields',
          'error'
        );
        return;
      }
      ...
      this.showNotification(
        `Sanitizer test logged: ${location} - ${concentration} ppm`,
        'success'
      );
    }
    ...
    addDishwasherTest() {
      ...
      const sanitizer = parseFloat(
        document.getElementById('dishwasher-sanitizer').value
      );
      const cycleTime = parseFloat(
      ...
      if (
        ...
        this.showNotification(
          'Please fill in all dishwasher test fields',
          'error'
        );
        return;
      }
      ...
      const status = [tempStatus, sanitizerStatus, cycleStatus].includes(
        'failed'
      )
        ? 'failed'
        : [tempStatus, sanitizerStatus, cycleStatus].includes('warning')
          ? 'warning'
          : 'passed';
