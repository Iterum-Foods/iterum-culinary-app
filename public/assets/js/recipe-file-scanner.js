...
class RecipeFileScanner {
  ...
  async scanLocalFiles() {
    ...
    try {
      ...
      const recipeFiles = files.filter(file =>
        this.isPotentialRecipeFile(file)
      );
  ...
  async searchGoogleDrive() {
    ...
    try {
      ...
      for (const keyword of this.recipeKeywords.slice(0, 3)) {
        // Limit to 3 keywords
  ...
  createBrowserUI(files) {
    ...
    let html =
      '<div class="recipe-files-browser" style="max-height: 600px; overflow-y: auto;">';
    ...
    for (const [source, sourceFiles] of Object.entries(filesBySource)) {
      ...
      const sourceLabel =
        source === 'local' ? '💻 Local Files' : '☁️ Google Drive';
