  ...
  class CalendarManager {
    ...
    loadFromLocalStorage() {
      ...
      if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
        ...
        this.recipeChanges =
          window.userDataManager.getUserData('recipe_changes') || [];
        this.equipmentMaintenance =
          window.userDataManager.getUserData('maintenance_log') || [];
      } else {
    ...
    saveToLocalStorage() {
      ...
      if (window.userDataManager && window.userDataManager.isUserLoggedIn()) {
        ...
        localStorage.setItem(
          'iterum_journal_entries',
          JSON.stringify(this.journalEntries)
        );
        localStorage.setItem(
          'iterum_recipe_changes',
          JSON.stringify(this.recipeChanges)
        );
        localStorage.setItem(
          'iterum_maintenance_log',
          JSON.stringify(this.equipmentMaintenance)
        );
      }
    }
    ...
    createDayElement(date) {
      ...
      dayDiv.className =
        'min-h-[120px] p-2 border border-gray-200 hover:bg-gray-50 cursor-pointer';
    ...
    getDayActivities(date) {
      ...
      const journalEntry = this.journalEntries.find(
      ...
      if (window.haccpManager) {
        const haccpData = window.haccpManager.temperatureReadings.filter(
          reading => new Date(reading.timestamp).toDateString() === dateString
        );
      ...
      const recipeChanges = this.recipeChanges.filter(
        change => new Date(change.date).toDateString() === dateString
      );
      ...
      const maintenance = this.equipmentMaintenance.filter(
    ...
    populateRecipeChanges(dateString) {
      ...
      const changes = this.recipeChanges.filter(
        change => new Date(change.date).toDateString() === dateString
      );
