...
class RecipeImportExport {
  ...
  parseRecipeFromText(text) {
    ...
    for (let line of lines) {
      ...
      if (line.match(/^\d+\.\s/) && currentSection === 'instructions') {
        const instruction = line.replace(/^\d+\.\s/, '');
        recipe.instructions.push(instruction);
      } else if (line.match(/^\d+\.\s/)) {
        // Recipe title (numbered lines outside Instructions section)
        ...
        recipe.prep_time =
          parseInt(line.replace('Prep Time:', '').replace('min', '').trim()) ||
          0;
      } else if (line.startsWith('Cook Time:')) {
        recipe.cook_time =
          parseInt(line.replace('Cook Time:', '').replace('min', '').trim()) ||
          0;
  ...
  cleanRecipe(recipe) {
    return {
      ...
      tags: Array.isArray(recipe.tags)
        ? recipe.tags.filter(t => t.trim())
        : ['Imported'],
      ingredients: Array.isArray(recipe.ingredients)
        ? recipe.ingredients.filter(i => i.name?.trim())
        : [],
      instructions: Array.isArray(recipe.instructions)
        ? recipe.instructions.filter(i => i.trim())
        : []
    };
  }
}
