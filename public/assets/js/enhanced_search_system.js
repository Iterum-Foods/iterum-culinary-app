...
class EnhancedSearchSystem {
    ...
    return items.filter(item => {
      ...
      if (filters.dietary.length > 0) {
        if (item.type === 'recipe' && item.dietary) {
          if (!filters.dietary.some(diet => item.dietary.includes(diet))) {
            return false;
          }
        }
      }
  ...
  sortResults(items, sortBy, sortOrder) {
    return items.sort((a, b) => {
      ...
      switch (sortBy) {
        ...
        case 'date':
          comparison =
            new Date(a.data.createdAt || 0) - new Date(b.data.createdAt || 0);
          break;
        case 'difficulty':
  ...
  getFilterOptions() {
    ...
    this.searchIndex.forEach(item => {
      if (item.difficulty) {
        options.difficulties.add(item.difficulty);
      }
      if (item.cuisine) {
        options.cuisines.add(item.cuisine);
      }
      if (item.dietary) {
        item.dietary.forEach(diet => options.dietary.add(diet));
      }
      if (item.data.category) {
        options.categories.add(item.data.category);
      }
      if (item.tags) {
  ...
  handleFilterChange(filter) {
    ...
    if (filter.multiple) {
      ...
      const selectedValues = Array.from(filter.selectedOptions).map(
        option => option.value
      );
   ...
   */
  async renderDirectoryBrowser(
    containerId = 'directory-browser',
    rootPath = '.'
  ) {
    const container = document.getElementById(containerId);
    if (!container) {
  ...
  renderDirectoryTree(dirs, basePath) {
    if (!dirs || dirs.length === 0) {
      return '<div class="dir-empty">No folders found</div>';
    }
    return `<ul class="dir-tree">${dirs
      .map(
        dir => `
            <li>
  ...
  async toggleFolder(el, path) {
    ...
    if (childrenDiv.style.display === 'none') {
      childrenDiv.innerHTML =
        '<div class="dir-loading"><span class="spinner"></span> Loading...</div>';
  ...
  displayFileSearchResults(results) {
    ...
    resultsContainer.querySelectorAll('.copy-btn').forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const filePath =
          btn.parentElement.querySelector('.file-path').textContent;
  ...
  displaySearchResults(searchResults) {
    ...
    resultsContainer.innerHTML = searchResults.results
  ...
  getAllIngredients() {
    // This should be implemented based on your actual data structure
    return window.ingredientLibrary
      ? window.ingredientLibrary.getAllIngredients()
      : [];
  }
  ...
  getAllEquipment() {
    // This should be implemented based on your actual data structure
    return window.equipmentManager
      ? window.equipmentManager.getAllEquipment()
      : [];
  }
...
function initializeEnhancedSearch() {
  if (
    ...
    typeof window.equipmentManager.getAllEquipment === 'function'
  ) {
