...
class CostCalculator {
  constructor() {
    ...
    this.overheadPercentage = 0.3; // 30% default overhead
    ...
    try {
      ...
      ingredients.forEach(ingredient => {
        ...
        if (ingredient.bestPrice) {
          ...
          price =
            ingredient.bestPrice.normalizedPrice ||
            ingredient.bestPrice.price ||
            price;
          unit = ingredient.bestPrice.baseUnit || unit;
          vendor = ingredient.bestPrice.vendor || vendor;
        } else if (
          ingredient.vendorPrices &&
          ingredient.vendorPrices.length > 0 &&
          window.vendorPriceComparator
        ) {
      ...
      vendors.forEach(vendor => {
        if (vendor.priceList) {
          vendor.priceList.forEach(item => {
            const key =
              item.ingredient?.toLowerCase() || item.item?.toLowerCase();
            if (key && item.price) {
              // Only use if not already set from ingredients database
              if (
                !this.ingredientPrices[key] ||
                item.price < this.ingredientPrices[key].price
              ) {
                this.ingredientPrices[key] = {
  ...
  calculateRecipeCost(recipe) {
    ...
    const yieldQuantity =
      parseInt(recipe.yieldQuantity || recipe.servings || recipe.yield) || 1;
    ...
    return {
      ...
      wasteTotal: ingredientCosts
        .reduce((sum, ing) => sum + (ing.wasteCost || 0), 0)
        .toFixed(2) // Phase 2: Total waste cost
    };
  }
  ...
  calculateIngredientCost(name, quantity, unit) {
        ...
        ingredients.find(
          ing =>
            (ing.name.toLowerCase() === key ||
              (ing.baseName && ing.baseName.toLowerCase() === key)) &&
            !ing.isVariant
        ...
        if (ingredient.bestPrice) {
          ...
          if (window.unitConverter) {
            ...
            if (caseUnitInBase !== null && caseUnitInBase > 0) {
              costPerBaseUnit =
                casePricing.pricePerCase /
                (caseUnitInBase * (casePricing.conversionFactor || 1));
            } else {
              costPerBaseUnit =
                casePricing.pricePerCase /
                (casePricing.caseSize * (casePricing.conversionFactor || 1));
            }
          } else {
            costPerBaseUnit =
              casePricing.pricePerCase /
    ...
    if (window.unitConverter) {
      convertedQuantity = window.unitConverter.convert(
        quantity,
        unit,
        priceData.unit
      );
  ...
  getVendorComparison(ingredientName) {
    ...
    vendors.forEach(vendor => {
      if (vendor.priceList) {
        const item = vendor.priceList.find(
          i =>
            i.ingredient?.toLowerCase() === key || i.item?.toLowerCase() === key
        );
        ...
        if (item && item.price) {
          comparison.push({
            ...
            unit: item.unit || 'lb',
            caseSize: item.caseSize,
            caseCost: item.caseSize
              ? parseFloat(item.price) * parseInt(item.caseSize)
              : null
          });
        }
      }
  ...
  createCostSummaryHTML(costData) {
    const hasMissingPrices =
      costData.missingPrices && costData.missingPrices.length > 0;
