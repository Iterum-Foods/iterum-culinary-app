...
class StartupLoadingManager {
  ...
  init() {
    console.log(
      '🕒 Startup loading configured for 3 seconds with user selection'
    );
  ...
  initializeStartupManager() {
    console.log(
      '🚀 Initializing startup manager with user selection requirement'
    );
  ...
  selectUserFromList() {
    console.log(
      '⚠️ selectUserFromList is deprecated - using inline user system'
    );
  ...
  setupEmergencyTimeout() {
    setTimeout(() => {
      if (!this.isHidden) {
        console.warn(
          '⚠️ EMERGENCY: Loading screen timeout reached (10s), forcing hide'
        );
        this.forceHideLoadingScreen();
      }
    }, this.maxWaitTime);
  ...
  hideLoadingScreenWithDelay() {
    ...
    if (remainingTime > 0) {
      console.log(
        `⏳ Loading screen will continue for ${remainingTime}ms more`
      );
      setTimeout(() => {
        this.forceHideLoadingScreen();
      }, remainingTime);
  ...
  ensurePageVisibility() {
    ...
    const mainElements = document.querySelectorAll(
      '.container, main, .main-content, #main-content, header, .header'
    );
    ...
    if (typeof Promise !== 'undefined') {
      ...
      Promise.all = function (promises) {
        return originalPromiseAll.call(this, promises).then(results => {
          console.log(
            '📦 Scripts finished loading, but maintaining 3-second minimum display'
          );
          return results;
        });
      };
...
console.log(
  '🚀 Startup Loading Manager initialized - 3 second loading screen active (max 10s)'
);
