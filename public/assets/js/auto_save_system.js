...
class AutoSaveManager {
  ...
  saveField(element) {
    if (!element || this.shouldExcludeField(element)) {
      return;
    }
  ...
  saveAllData(immediate = false) {
    ...
    // Save all form data
    const allDataKey =
      this.options.storagePrefix + `${this.currentPageId}_all_data`;
    const saveData = {
      forms: formData,
      timestamp: Date.now(),
      pageId: this.currentPageId,
  ...
  restoreData(savedData) {
    ...
    Object.entries(savedData.forms).forEach(([formId, formData]) => {
      const form =
        ...
        document.forms[parseInt(formId.replace('form_', ''))];
      if (!form) {
        return;
      }
      ...
      Object.entries(formData).forEach(([fieldName, value]) => {
        ...
        if (!field || this.shouldExcludeField(field)) {
          return;
        }
