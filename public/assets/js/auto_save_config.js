...
function initializeAutoSave() {
  ...
  if (window.AutoSaveManager) {
    ...
    console.warn(
      'AutoSaveManager not loaded. Please include auto_save_system.js'
    );
  }
}
...
function addRecipeDeveloperFeatures() {
  ...
  document.addEventListener('input', e => {
    if (
      e.target.matches('.recipe-step, .ingredient-input, .instruction-input')
    ) {
...
function addCriticalFieldIndicator(element) {
  if (!element || element.dataset.criticalIndicatorAdded) {
    return;
  }
