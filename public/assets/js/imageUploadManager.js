...
class ImageUploadManager {
  constructor(options = {}) {
    ...
    this.defaultImageUrl =
      options.defaultImageUrl || '/static/images/default-placeholder.jpg';
  ...
  validateFile(file) {
    ...
    if (!this.allowedTypes.includes(file.type)) {
      errors.push(
        `File type ${file.type} not allowed. Supported types: ${this.allowedTypes.join(', ')}`
      );
    }
  ...
  async uploadRecipePrimaryImage(recipeId, file, progressCallback) {
    return this.uploadImage(
      `${this.apiBaseUrl}/images/recipe/${recipeId}/primary`,
      file,
      progressCallback
    );
  }
  ...
  async uploadRecipeGalleryImages(recipeId, files, progressCallback) {
    ...
    return this.uploadFormData(
      `${this.apiBaseUrl}/images/recipe/${recipeId}/gallery`,
      formData,
      progressCallback
    );
  }
  ...
  async uploadRecipeStepImage(recipeId, stepNumber, file, progressCallback) {
    return this.uploadImage(
      `${this.apiBaseUrl}/images/recipe/${recipeId}/step/${stepNumber}`,
      file,
      progressCallback
    );
  }
  ...
  async uploadIngredientImage(ingredientId, file, progressCallback) {
    return this.uploadImage(
      `${this.apiBaseUrl}/images/ingredient/${ingredientId}/primary`,
      file,
      progressCallback
    );
  }
  ...
  async getRecipeImages(recipeId) {
    const response = await fetch(
      `${this.apiBaseUrl}/images/recipe/${recipeId}/images`,
      {
  ...
  async deleteRecipeGalleryImage(recipeId, filename) {
    const response = await fetch(
      `${this.apiBaseUrl}/images/recipe/${recipeId}/gallery/${filename}`,
      {
        method: 'DELETE',
        headers: this.token ? { Authorization: `Bearer ${this.token}` } : {}
      }
  ...
  createImageGallery(container, images, options = {}) {
    ...
    images.forEach((image, index) => {
      ...
      img.src =
        showThumbnails && image.thumbnail_url ? image.thumbnail_url : image.url;
