...
(function () {
  ...
  console.log(
    '🔄 Header Sync Loader: Starting automatic header synchronization...'
  );
  ...
  function handleVisibilityChange() {
    if (!document.hidden && window.headerUserSync) {
      ...
      setTimeout(() => {
        if (
          window.headerUserSync &&
          typeof window.headerUserSync.refresh === 'function'
        ) {
  ...
  function handleStorageChange(event) {
    if (event.key === 'current_user' || event.key === 'session_active') {
      ...
      setTimeout(() => {
        if (
          window.headerUserSync &&
          typeof window.headerUserSync.syncUserDisplay === 'function'
        ) {
