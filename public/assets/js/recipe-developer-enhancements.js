...
class RecipeDeveloperEnhancements {
  ...
  setupProgressTracking() {
    ...
    sections.forEach(section => {
      const inputs = document.querySelectorAll(
        `[data-section="${section}"] input, [data-section="${section}"] textarea, [data-section="${section}"] select`
      );
  ...
  updateProgressBar() {
    ...
    sections.forEach((section, index) => {
      const stepEl = document.querySelector(
        `.progress-step[data-step="${section}"]`
      );
      if (stepEl) {
        ...
        if (this.completedSections.has(section)) {
          ...
          const firstIncomplete = sections.find(
            s => !this.completedSections.has(s)
          );
  ...
  collectInstructions() {
    const instructions = [];
    document
      .querySelectorAll('.step-item textarea, .step-item input[type="text"]')
      .forEach(input => {
      ...
      try {
        ...
        if (hoursSince < 24) {
          const restore = confirm(
            `Found autosaved draft from ${savedTime.toLocaleString()}.\n\nWould you like to restore it?`
          );
  ...
  restoreFormData(data) {
    ...
    if (data.servings) {
      document.getElementById('recipe-servings').value = data.servings;
    }
    if (data.time) {
  ...
  showAutosaveIndicator(status) {
    ...
    if (status === 'saving') {
      ...
      indicator.innerHTML =
        '<div class="autosave-spinner"></div><span>Saving draft...</span>';
