...
class DataTaggingSystem {
  ...
  untagItem(projectId, itemType, itemId) {
    if (this.tags.has(projectId)) {
      ...
      if (index > -1) {
        ...
        this.saveTags();
        console.log(
          `✅ Untagged ${itemType} ${itemId} from project ${projectId}`
        );
        return true;
      }
    }
  ...
  bulkTagItems(projectId, itemType, itemIds) {
    ...
    this.saveTags();
    console.log(
      `✅ Bulk tagged ${itemIds.length} ${itemType} with project ${projectId}`
    );
    return true;
  }
  ...
  bulkUntagItems(projectId, itemType, itemIds) {
    if (this.tags.has(projectId)) {
      ...
      this.saveTags();
      console.log(
        `✅ Bulk untagged ${itemIds.length} ${itemType} from project ${projectId}`
      );
      return true;
    }
    return false;
  ...
  getProjectStats(projectId) {
    ...
    return {
      ...
      total:
        items.ingredients.length + items.recipes.length + items.menus.length
    };
  }
