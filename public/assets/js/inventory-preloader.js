...
class InventoryPreloader {
  ...
  async preloadInventory() {
    try {
      ...
      console.log(
        `📋 Found ${chefWarehouseIngredients.length} Chef's Warehouse ingredients`
      );
      ...
      console.log(
        `✅ Preloaded ${chefWarehouseIngredients.length} items into inventory`
      );
      console.log(`📊 Total inventory items: ${inventory.length}`);

      // Dispatch event
      window.dispatchEvent(
        new CustomEvent('inventoryPreloaded', {
          detail: {
  ...
  getInventoryStats(userId) {
    ...
    const stats = {
      totalItems: inventory.length,
      chefWarehouseItems: inventory.filter(i => i.vendor === "Chef's Warehouse")
        .length,
      inStock: inventory.filter(i => i.status === 'in-stock' && i.quantity > 0)
        .length,
