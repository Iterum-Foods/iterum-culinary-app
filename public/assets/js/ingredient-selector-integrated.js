...
class IngredientSelectorIntegrated {
    ...
    /**
     * Search USDA database and add to selector
     * @param {string} query - Search query
     * @param {number} maxResults - Max results to fetch
     */
    async searchUSDA(query, maxResults = 20) {
        if (!window.usdaApiClient || !window.usdaApiClient.hasApiKey()) {
            return { success: false, message: 'USDA API key not set' };
        }

        try {
            const result = await window.usdaApiClient.searchFoods(query, { pageSize: maxResults });
            
            if (result.foods && result.foods.length > 0) {
                const usdaIngredients = result.foods.map(food => 
                    window.usdaApiClient.transformToIngredient(food)
                );
                
                // Add to ingredients list temporarily for this search
                return {
                    success: true,
