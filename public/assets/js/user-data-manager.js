...
class UserDataManager {
  ...
  updateCurrentProject() {
    ...
    if (window.unifiedProjectSelector) {
      ...
      const storedProjectId =
        localStorage.getItem(`iterum_current_project_user_${this.userId}`) ||
    ...
    try {
      ...
      if (dataType !== 'projects' && options.filterByProject !== false) {
        ...
        if (projectId === 'master') {
          ...
          console.log(
            `📊 Loaded ${filtered.length} ${dataType} for user (Master Project - ALL DATA)`
          );
        } else {
          ...
          console.log(
            `📊 Loaded ${filtered.length} ${dataType} for project "${projectId}" (filtered from ${beforeFilter} total)`
          );
        }
      } else {
        console.log(
          `📊 Loaded ${filtered.length} ${dataType} for user (no project filter)`
        );
      }
    ...
    for (const dataType of this.dataCollections) {
      try {
        ...
        data.forEach(item => {
          ...
          if (!item.userId && dataType !== 'ingredients') {
            invalidItems++;
          }
        });
        ...
        if (invalidItems > 0) {
          issues.push(
            `${dataType}: ${invalidItems} items missing id or userId`
          );
  ...
  cleanOldBackups() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (
        key &&
        key.startsWith(`backup_${this.userId}_`) &&
        key !== `backup_${this.userId}_latest`
      ) {
        backupKeys.push(key);
      }
    }
  ...
  setupDataValidation() {
    ...
    this.saveData = (dataType, data, options = {}) => {
      ...
      if (!validationResult.valid) {
        console.error(
          `❌ Data validation failed for ${dataType}:`,
  ...
  async importUserData(file) {
    try {
      ...
      const totalItems = Object.values(importData.data).reduce(
      ...
      if (
        !confirm(
          `Import ${totalItems} items (${dataTypes.join(', ')}) from ${importData.userEmail}?`
        )
      ) {
        return false;
      }
  ...
  clearAllUserData(confirm = true) {
    if (
      confirm &&
      !window.confirm('⚠️ Clear ALL your data? This cannot be undone!')
    ) {
      return false;
    }
