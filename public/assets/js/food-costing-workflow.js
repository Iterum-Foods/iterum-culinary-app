...
class FoodCostingWorkflow {
  ...
  calculateCostPerYieldUnit(actualIngredientCosts, yieldQuantity) {
    const totalActualCost = actualIngredientCosts.reduce(
      (sum, cost) => sum + cost,
      0
    );
    ...
    menuItems.forEach(menuItem => {
      ...
      recipe.ingredients.forEach(ingredient => {
        ...
        const quantityNeeded =
          parseFloat(ingredient.quantity || ingredient.amount) || 0;
  ...
  performVarianceAnalysis(
    ...
    const actualUsage = this.calculateActualUsage(
      openingInventory,
      purchases,
      closingInventory
    );
    ...
    return {
      ...
      variancePercentage:
        theoreticalUsage > 0 ? (variance / theoreticalUsage) * 100 : 0,
