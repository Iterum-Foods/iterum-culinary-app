...
class EnhancedMenuImport {
  ...
  displaySummary(items) {
    ...
    const avgPrice =
      items.reduce((sum, i) => sum + (Number(i.price) || 0), 0) / totalItems ||
      0;
        ...
        return `
            <div class="parse-item" data-item-index="${index}" style="border: 2px solid var(--iterum-border-light); border-radius: 12px; padding: 20px; margin-bottom: 16px; background: white;">
                ...
                ${
                  item.ingredients
                    ? `
                    <div class="parse-item-ingredients" style="color: var(--iterum-text-tertiary); margin-bottom: 12px; font-size: 0.875rem; padding: 8px; background: #f0f9ff; border-radius: 6px;">
                        <strong>Ingredients:</strong> ${this.escapeHtml(item.ingredients)}
                    </div>
                `
                    : ''
                }
                ${
                  item.recipeInfo
                    ? `
                    <div class="parse-item-recipe" style="color: var(--iterum-text-tertiary); margin-bottom: 12px; font-size: 0.875rem; padding: 8px; background: #fef3c7; border-radius: 6px;">
                        <strong>Dish Build:</strong> ${this.escapeHtml(item.recipeInfo)}
                    </div>
                `
                    : ''
                }
                ${
                  item.allergens && item.allergens.length > 0
                    ? `
                    <div class="parse-item-allergens" style="color: #991b1b; margin-bottom: 12px; font-size: 0.875rem; padding: 8px; background: #fee2e2; border-radius: 6px;">
                        <strong>Allergens:</strong> ${item.allergens.map(a => this.escapeHtml(a)).join(', ')}
                    </div>
                `
                    : ''
                }
  ...
  linkRecipe(index) {
    ...
    if (itemElement) {
      const actionsDiv = itemElement.querySelector(
        '.parse-item-recipe-actions'
      );
      if (actionsDiv) {
        actionsDiv.innerHTML =
          '<div class="recipe-linked-badge">✓ Recipe Linked</div>';
      }
    }
  ...
  createRecipe(index) {
    ...
    if (itemElement) {
      ...
      if (actionsDiv) {
        actionsDiv.innerHTML =
          '<div class="recipe-linked-badge">✓ Creating Recipe...</div>';
      }
    }
  ...
  async applyDishMappingResults() {
    ...
    if (unmapped.length > 0) {
      alert(
        `⚠️ Please map all dishes before applying.\n\n${unmapped.length} dish(es) still need mapping.`
      );
      return;
    }

    // Validate existing mappings have selected items
    const invalidExisting = this.importedItems.filter(
      item => item.mappingType === 'existing' && !item.mappedToItemId
    );
    if (invalidExisting.length > 0) {
      alert(
        `⚠️ Please select existing dishes for all "Link to Existing" mappings.\n\n${invalidExisting.length} mapping(s) incomplete.`
      );
      return;
    }
    ...
    try {
      ...
      for (const item of this.importedItems) {
        if (item.mappingType === 'existing') {
          ...
          const existingItem = window.menuItemsDatabase?.getMenuItem(
            item.mappedToItemId
          );
          if (existingItem) {
            // Link item to this menu
            window.menuItemsDatabase.linkToMenu(
              item.mappedToItemId,
              targetMenuId
            );
            ...
            await window.enhancedMenuManager.addMenuItem(
              {
                ...
                recipeInfo: item.recipeInfo || null,
      ...
      alert(
        `Error: ${error.message || 'Failed to apply dish mappings. Check console for details.'}`
      );
    }
  }
  ...
  async createRecipes(menuId = null) {
    ...
    if (itemsNeedingRecipes.length === 0) {
      return;
    }
  ...
  getCurrentUserId() {
    try {
      const user =
        (window.userSystem &&
          window.userSystem.getCurrentUser &&
          window.userSystem.getCurrentUser()) ||
        null;
