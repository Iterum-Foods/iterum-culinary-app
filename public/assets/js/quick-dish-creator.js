...
class QuickDishCreator {
   ...
   */
  addCreatorStyles() {
    if (document.getElementById('quick-dish-creator-styles')) {
      return;
    }
  ...
  loadDishData() {
    ...
    document.getElementById('dish-description').value =
      this.currentDish.description || '';

    // Load recipe information
    document.getElementById('dish-cuisine').value =
      this.currentDish.cuisine || '';
    document.getElementById('dish-difficulty').value =
      this.currentDish.difficulty || 'medium';
    document.getElementById('dish-prep-time').value =
      this.currentDish.prepTime || '';
    document.getElementById('dish-cook-time').value =
      this.currentDish.cookTime || '';
  ...
  loadIngredients() {
    if (
      ...
      this.currentDish.ingredients.length > 0
    ) {
  ...
  loadInstructions() {
    if (
      this.currentDish.instructions &&
      this.currentDish.instructions.length > 0
    ) {
  ...
  autoGenerateInstructions() {
    ...
    const category = document.getElementById('dish-category').value;
    const instructions = this.generateInstructionsFromIngredients(
      this.ingredients,
      category
    );
  ...
  updateCostAnalysis() {
    ...
    document.getElementById('ingredient-cost').textContent =
  ...
  saveAsTemplate() {
    ...
    const templates = JSON.parse(
      localStorage.getItem('dish_templates') || '[]'
    );
  ...
  collectDishData() {
    ...
    return {
      ...
      prepTime:
        parseInt(document.getElementById('dish-prep-time').value) || null,
      cookTime:
  ...
  setupEventListeners() {
    ...
    document.addEventListener('click', e => {
      if (
        ...
        e.target.id === 'quick-dish-creator-modal'
      ) {
        this.closeCreator();
      }
    });
    ...
    document.addEventListener('keydown', e => {
      if (
        e.key === 'Escape' &&
        document.getElementById('quick-dish-creator-modal')
      ) {
        this.closeCreator();
      }
    });
    ...
    return {
      ...
      'prep-recipe': {
        instructions: [
          ...
          {
            step: 2,
            instruction: 'Prepare ingredients as needed (chop, dice, etc.)'
          },
  ...
  generateInstructionsFromIngredients(ingredients, category) {
    ...
    const instructions = [
      ...
      {
        step: 3,
        instruction: 'Cook ingredients according to their cooking times'
      },
  ...
  calculateIngredientCost() {
    ...
    return this.ingredients.length * 0.5; // $0.50 per ingredient estimate
  }
  ...
  calculateLaborCost() {
    const prepTime =
      parseInt(document.getElementById('dish-prep-time').value) || 0;
    const cookTime =
      parseInt(document.getElementById('dish-cook-time').value) || 0;
