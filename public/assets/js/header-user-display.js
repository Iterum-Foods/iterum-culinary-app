...
(function () {
  ...
  async function updateHeaderUserDisplay() {
    ...
    const userNameElements = document.querySelectorAll(
      '#current-user, #header-user-name, #dropdown-user-name'
    );
    ...
    const avatarElements = document.querySelectorAll(
      '#user-avatar-initial, #user-avatar-text'
    );
    ...
    const userRoleElements = document.querySelectorAll(
      '#dropdown-user-role, .header-user-role'
    );
    userRoleElements.forEach(el => {
      if (el) {
        el.textContent = user.type === 'trial' ? 'Trial User' : 'Chef';
      }
    });
