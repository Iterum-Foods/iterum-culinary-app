...
class CloudDataSync {
  ...
  async syncFromCloud() {
    ...
    try {
      ...
      // Sync recipes
      const recipesCount = await this.syncCollectionFromCloud(
        'recipes',
        `recipes_${this.userId}`
      );
      totalSynced += recipesCount;

      // Sync menus
      const menusCount = await this.syncCollectionFromCloud(
        'menus',
        `menus_${this.userId}`
      );
      ...
      // Sync projects
      const projectsCount = await this.syncCollectionFromCloud(
        'projects',
        `iterum_projects_${this.userId}`
      );
      totalSynced += projectsCount;

      // Sync ingredients (user-specific additions)
      const ingredientsCount = await this.syncCollectionFromCloud(
        'ingredients',
        `user_ingredients_${this.userId}`
      );
      totalSynced += ingredientsCount;

      // Sync vendors
      const vendorsCount = await this.syncCollectionFromCloud(
        'vendors',
        'vendors'
      );
      ...
      // Sync equipment
      const equipmentCount = await this.syncCollectionFromCloud(
        'equipment',
        `equipment_${this.userId}`
      );
      ...
      if (totalSynced > 0) {
        this.showSyncNotification(
          `✅ Downloaded ${totalSynced} items from cloud`
        );
      }

      // Dispatch event
      window.dispatchEvent(
        new CustomEvent('cloudSyncComplete', {
  ...
  async syncCollectionFromCloud(firestoreCollection, localStorageKey) {
    try {
      ...
      if (snapshot.empty) {
        console.log(
          `ℹ️ No ${firestoreCollection} found in cloud for this user`
        );
        return 0;
      }
      ...
      console.log(
        `✅ Synced ${items.length} ${firestoreCollection} from cloud (${mergedItems.length} total after merge)`
      );
  ...
  async syncToCloud(dataType, localStorageKey) {
    ...
    try {
      ...
      for (const item of items) {
        ...
        item.userEmail =
          this.userEmail || window.authManager?.currentUser?.email;
  ...
  setupAutoSync() {
    ...
    window.addEventListener('beforeunload', () => {
      // Always sync on exit (use sendBeacon for reliable sync on page close)
  ...
  async syncAllDataToCloud() {
    ...
    try {
      ...
      totalUploaded += await this.syncToCloud(
        'recipes',
        `recipes_${this.userId}`
      );
      ...
      console.log(
        `✅ All data synced to cloud - Uploaded ${totalUploaded} items`
      );

      if (totalUploaded > 0) {
        this.showSyncNotification(
          `✅ Uploaded ${totalUploaded} items to cloud`
        );
      }
    } catch (error) {
      console.error('❌ Cloud sync error:', error);
  ...
  syncAllDataToCloudImmediate() {
    ...
    fetch(
      `https://firestore.googleapis.com/v1/projects/iterum-culinary-app2/databases/(default)/documents:commit`,
      {
        method: 'POST',
        keepalive: true,
  ...
  setupLocalStorageListeners() {
    ...
    console.log(
      '✅ localStorage listeners active - instant cloud sync on save'
    );
  }
  ...
  getDataTypeFromKey(key) {
    if (key.includes('recipes_')) {
      return 'recipes';
    }
    if (key.includes('menus_')) {
      return 'menus';
    }
    if (key.includes('iterum_projects_')) {
      return 'projects';
    }
    if (key.includes('equipment_')) {
      return 'equipment';
    }
    if (key === 'vendors') {
      return 'vendors';
