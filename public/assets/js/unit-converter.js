...
class UnitConverter {
  ...
  getUnitCategory(unit) {
    ...
    if (this.weightConversions[normalized]) {
      return 'weight';
    }
    if (this.volumeConversions[normalized]) {
      return 'volume';
    }
    if (this.countUnits.includes(normalized)) {
      return 'count';
    }
  ...
  convert(value, fromUnit, toUnit) {
    ...
    if (fromCategory !== toCategory) {
      console.warn(
        `⚠️ Cannot convert ${fromUnit} to ${toUnit} - different categories`
      );
      return null;
    }
    ...
    Object.keys(byCategory).forEach(category => {
      ...
      comparisons.push({
        ...
        priceRange: {
          ...
          difference:
            normalized[normalized.length - 1].normalized.price -
            normalized[0].normalized.price,
          percentDifference: (
            ((normalized[normalized.length - 1].normalized.price -
              normalized[0].normalized.price) /
              normalized[0].normalized.price) *
  ...
  calculateIngredientCost(ingredient, recipeQuantity, recipeUnit) {
    ...
    const recipeQuantityInBase = this.convert(
      recipeQuantity,
      recipeUnit,
      baseUnit
    );
    ...
    if (recipeQuantityInBase === null) {
      console.warn(
        `⚠️ Cannot convert recipe quantity ${recipeQuantity} ${recipeUnit} to base unit ${baseUnit}`
      );
      return null;
    }
    ...
    if (ingredient.casePricing) {
      const { pricePerCase, caseSize, caseUnit, conversionFactor } =
      ...
      if (caseSizeInBase !== null && caseSizeInBase > 0) {
        costPerBaseUnit =
          pricePerCase / (caseSizeInBase * (conversionFactor || 1));
      }
    }
