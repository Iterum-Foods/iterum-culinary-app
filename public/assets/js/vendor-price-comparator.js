...
class VendorPriceComparator {
  ...
  updateBestPrice(ingredient) {
    ...
    const normalized = ingredient.vendorPrices
      .map(vp => ({
        ...vp,
        normalizedPrice: this.unitConverter.convertPrice(
          vp.price,
          vp.unit,
          baseUnit
        )
    ...
    ingredient.priceComparison = {
      ...
      priceRange: {
        ...
        difference:
          normalized[normalized.length - 1].normalizedPrice -
          normalized[0].normalizedPrice,
        percentSavings: (
          ((normalized[normalized.length - 1].normalizedPrice -
            normalized[0].normalizedPrice) /
            normalized[normalized.length - 1].normalizedPrice) *
