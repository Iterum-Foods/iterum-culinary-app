...
class ErrorHandler {
  ...
  handleAPIError(error, context = {}) {
    ...
    if (
      status === 404 &&
      url &&
      (url.includes('/api/') || url.includes('/health'))
    ) {
    ...
    switch (status) {
      ...
      case 403:
        console.log(
          '🚫 Forbidden - User lacks permission or not authenticated'
        );
        break;
      case 404:
  ...
  logAPIError(errorInfo) {
    try {
      const errorLog = JSON.parse(
        localStorage.getItem('api_error_log') || '[]'
      );
  ...
  logSecurityViolation(event) {
    ...
    try {
      const violations = JSON.parse(
        localStorage.getItem('security_violations') || '[]'
      );
  ...
  async checkBackendStatus() {
    try {
      ...
      this.showUserMessage(
        'Backend server is not available. Running in offline mode.',
        'info'
      );
    }
  }
