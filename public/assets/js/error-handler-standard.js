...
class StandardErrorHandler {
  ...
  showUserMessage(errorInfo) {
    ...
    if (window.toastNotifications) {
      window.toastNotifications.error(this.getUserFriendlyMessage(errorInfo), {
        duration: 5000
      });
    } else {
      // Fallback to alert
  ...
  getUserFriendlyMessage(errorInfo) {
    const messages = {
      ...
      FirebaseError:
        'Unable to save data to cloud. Your data is saved locally.',
      QuotaExceededError:
    ...
    return (
      messages[errorInfo.type] || `An error occurred: ${errorInfo.message}`
    );
  }
