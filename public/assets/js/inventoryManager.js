...
function filterAndSortInventory() {
  ...
  if (window.showingExpiring) {
    filtered = filtered.filter(
      item => item.expiry_date && daysUntil(item.expiry_date) <= 3
    );
  }
  // Sort
  filtered.sort((a, b) => {
...
function showInventoryModal(editIdx = null) {
  ...
  document.getElementById('inventory-modal-title').textContent =
    editIdx === null ? 'Add Inventory' : 'Edit Inventory';
  ...
  if (editIdx !== null) {
    ...
    document.getElementById('inventory-date-received').value =
      item.date_received;
    ...
    document.getElementById('inventory-date-received').value = new Date()
      .toISOString()
      .split('T')[0];
...
function handleExcelImport(e) {
  ...
  reader.onload = function (evt) {
    ...
    if (file.name.endsWith('.csv')) {
      ...
      if (rows.length > 1 && rows[0][0].toLowerCase().includes('ingredient')) {
        rows.shift();
      }
            ...
            page.getTextContent().then(function (textContent) {
              ...
              if (pageNum < pdf.numPages) {
                ...
                const tableLines = lines.filter(
                  l => l.split(',').length >= 4 || l.split(/\s{2,}/).length >= 4
                );
...
function setupInventoryEvents() {
  ...
  document.getElementById('inventory-ingredient').onchange =
    handleIngredientChange;
  document.getElementById('bulk-import-btn').onclick = showBulkImportModal;
  document.getElementById('close-bulk-import-modal').onclick =
    hideBulkImportModal;
...
document
  .getElementById('bulk-import-inventory-pdf')
    ...
    if (!document.getElementById('inventory-pdf-import-btn')) {
      ...
      btn.addEventListener('click', function () {
        ...
        for (let row of tableRows.slice(1)) {
          ...
          map.forEach((field, i) => {
            if (field !== 'Ignore') {
              obj[field.toLowerCase().replace(/ /g, '_')] = row[i] || '';
            }
          });
          if (obj.name) {
            imported.push(obj);
